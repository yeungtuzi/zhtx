...
void slash(object victim, object player)
{
                int gil_flag, lea, damage, wound, gin_sen_damage, dp, v_exp, s, suck, heal,damage_bonus, self_damage;
                ...
                if (flag)
                {
                        ...
                        message("vision", CYN"\n"+YEL"只见此人身着一袭紧身黑衣，以黑巾蒙面，身后还背负一个宽大的包裹，正是传说中的兵器贩子。\n", environment());
                        message("sound", HIW"\n"NOR, environment());
                        ...
                        gil_flag++;
                        gil_flag = gil_flag + 2 * random(lea);
                        if (gil_flag>1000)
                        {
                                gil_flag=1000;
                        }
                ...
                message("vision", CYN"\n"+CYN"兵器贩子如电光石火般地在" + victim_name + CYN"身旁一晃，从背后包裹中随手抽出"NOR+sword_name+CYN"一挥。\n", environment());
                ...
                //              message("vision", HIW"\n只见一道雪亮的白光划过" +victim_name+ HIW"的胸膛\n"NOR, environment());
                        ...
                        victim->reset_action();
                                ...
                                case 2: victim->start_call_out( (: call_other, __FILE__, "remove_effect", victim :), 50);
                                                ...
                                                break;
                                //破残：冰冻
                                case 3: victim->start_busy(5); 
                                                tell_object(victim, CYN"你只觉得一阵严寒袭体，刹那间，仿佛置身冰窖，一时浑身僵硬，不能动弹\n");
                                                break;
                                //刈鹿刀：
                                case 4: suck = victim->query("eff_kee")/20;
                                                if (suck<1)
                                                {
                                                        suck=1;
                                                }
                                                ...
                                                break;
                                //灵空剑
                                case 5: damage_bonus = victim->query("eff_kee")/10;
                                                if (damage_bonus<1)
                                                {
                                                        damage_bonus=1;
                                                }
                                                self_damage = damage_bonus/10;
                                                if (self_damage>50)
                                                {
                                                        self_damage=50;
                                                }
                                ...
                                message("vision", CYN"\n兵器贩子反手将"NOR + sword_name + CYN"插回背后包裹，姿势潇洒已极。\n"NOR, environment());
                        }
                        else 
                        {
                                message("vision", CYN"\n却见" + victim_name + CYN"冷笑一声，身子微微一侧，在千钧一发之刻，躲过了这致命一击\n"NOR, environment());
                                message("vision", CYN"\n兵器贩子与" + victim_name + CYN"擦身而过的一刹那间，" + victim_name + CYN"手上似乎多了一物，轻轻一晃\n"NOR, environment());
                                message("vision", "\n"NOR + sword_name + HIW"旋转着从天空坠落插在"+ victim_name +HIW"身边的地上，颤动不已\n"NOR, environment());
                                message("vision", CYN"\n兵器贩子胸前忽地喷出一股鲜血，洒向半空中，形成一片凄美的红雨\n"NOR, environment());
