...
int main(object me, string arg)
{
	...
	if( (dest->query_max_encumbrance() < 1) )
		return notify_fail(sprintf("%s里不能放东西！\n" , dest->name()));

	if( dest->query("no_get_from")) 
		return notify_fail("还是不要惊醒梦中人吧。\n");
	if( dest->is_character() && !living(dest) )
		return notify_fail("那里不能放东西。\n");
	...
	if(!objectp(obj = present(item, me)))
		return notify_fail("你身上没有这样东西。\n");
	if(obj->is_character()) 
		return notify_fail("你不能把活人或活物放进去！\n");
	return do_put(me, obj, dest);
}
