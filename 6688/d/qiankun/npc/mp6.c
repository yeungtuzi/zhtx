...
HIC"“苍天不负我命，终于让我等到了这一天，好孩子，这把天遁神剑今后就交给你了！”\n",NOR,
HIR"“一定要好好珍惜它，记住，‘剑在人在，剑亡人亡！’”\n",NOR,
HIG"“听说我的师兄开阳后来归隐去了侠客岛，如果有机会的话将来你去找他，”\n",NOR,
...
void create()
{
        ...
        set_skill("sword",220);
        set_skill("quanzhen-jian",220);
        set_skill("tonggui-jian",220);
        set_skill("array", 220);
        set_skill("force", 220);
        set_skill("xiantian-gong", 220);
        set_skill("dodge", 220);
        set_skill("fx-step", 220);
        set_skill("cuff", 220);
        set_skill("parry", 220);
        set_skill("tiangang-beidou",220);
        set_skill("yangxin-quan",220);
        ...
        create_family("全真派", 2, "无名前辈");
        set("title", HIB"乾坤教囚徒"NOR);
        set("nickname",HIR "逝去的辉煌，永世的神伤"NOR);
        setup();

        carry_object("/d/qiankun/obj/weapon/tiandun-shenjian")->wield();
}
...
void die()
{
       ...
       command("sigh");
       if(me->query_skill("tonggui-jian",1) > 199) 
       {
       me->remove_all_enemy();
       ob->remove_all_enemy();
        revive();
...
{        
        object weapon = new("/d/qiankun/obj/weapon/tiandun-shenjian");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("tiandun shenjian",who) )) 
                {
                weapon->move(who);
                }
