...
int wield()
{
       ...
       me = this_player();
       ob = this_object();
        if( this_player()->query_skill("cloudstaff",1) < 240)
        {
                return notify_fail("你的白云杖法等级太低了，用不了这件兵器。\n");
        }
...
int do_zhaohuan()
{
        ...
        seteuid(getuid());
        if( random(5)<3 || (me->query_skill("cloudstaff",1) < 200))
