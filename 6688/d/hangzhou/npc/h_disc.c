...
void create()
{
	...
	"xiaolin" : ({
		(: command,"smile" :),
		(: command,"say 就是他把我拖下凡，害得我都不好惩罚温暖了。\n" :),
		}),
	"道可道" : ({
		(: command,"say 他和我同为这个站的天神，我出国之后他就更忙了。\n" : ),
		(: command,"sigh" :),
		}),
	"dao" : ({
		(: command,"say 他和我同为这个站的天神，我出国之后他就更忙了。\n" : ),
	...
	carry_object("/d/hangzhou/npc/obj/wqi")->wield();
}
...
string ask_moon()
{
	...
	if(ob->query_temp("moon/disc"))
	{
		command("hmm "+ob->query("id"));
			return "........你不是已经问过了吗？\n";
	}
	ob->set_temp("moon/disc",1);
	command("smile");
