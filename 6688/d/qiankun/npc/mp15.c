...
HIC"“苍天不负我命，终于让我等到了这一天，好孩子，这把混世魔剑今后就交给你了！”\n",NOR,
HIR"“一定要好好珍惜它，记住，‘剑在人在，剑亡人亡！’”\n",NOR,
HIG"“听说我的师姐不知火舞后来归隐去了侠客岛，如果有机会的话将来你去找他，”\n",NOR,
...
void create()
{
       ...
       //         (: exert_function, "jieti" :),
        ...
        set_skill("force", 220);
        set_skill("tianmo-dafa", 220);
        set_skill("dodge", 220);
        set_skill("mo-shan-jue", 220);
        set_skill("strike", 220);
        set_skill("mo-zhang-jue", 220);
        set_skill("parry", 220);
        set_skill("sword", 220);
        set_skill("mo-jian-jue", 220);  
        set_skill("move",220);
        ...
        create_family("日月神教", 2, "无名前辈");
        set("title", HIB"乾坤教囚徒"NOR);
        set("nickname",HIR "逝去的辉煌，永世的神伤"NOR);
        setup();

        carry_object("/d/qiankun/obj/weapon/hunshi-mojian")->wield();
}
...
void die()
{
       ...
       command("sigh");
       if(me->query_skill("mo-jian-jue",1) > 199) 
       {
       me->remove_all_enemy();
       ob->remove_all_enemy();
        revive();
...
{        
        object weapon = new("/d/qiankun/obj/weapon/hunshi-mojian");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("hunshi mojian",who) )) 
                {
                weapon->move(who);
                }
