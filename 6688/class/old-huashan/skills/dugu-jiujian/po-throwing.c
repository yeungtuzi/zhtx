...
int perform(object me, object target)
{
        ...
        seteuid(ROOT_UID);
        if( !me->is_fighting() )
        	return notify_fail("「破箭式」只能在战斗中施用。\n");        	                     
	if( me->is_busy())
	...
	if (!objectp(ob = me->query_temp("weapon"))
        ...
        if( skill < 150 ) return notify_fail("你的「独孤九剑」招式不够娴熟，使不出「破箭式」。\n");        	                     
        if( me->query_skill("throwing",1)<150) return notify_fail("你的基本功不够扎实，无法使用「破箭式」。\n");
          
        if( me->query("betrayer"))
            return notify_fail("你回想起以前在师门中所曾经受过的好处，再也无法使出独孤九剑来了。\n");
            
        if(!check_weapon(target,"throwing")) 
            return notify_fail("对手手中并没有暗器，你如何使用「破箭式」？\n");
        
        message_vision(HIY"$N使出独孤九剑「破箭式」，静气凝神，仔细观察$n的一举一动，试图找出$n的破绽.\n" NOR, me,target);
...
int do_prepare(object me,object target)
{
	...
	if( left_busy > 3 )
	{
		...
		{
		        tell_object(me,"对手手上已经没有暗器了，没法使用「破箭式」。\n");
