// /d/xizang/class/bad/npc/obj/noodle.c
...
void create()
{
        set_name("面条", ({ "mian tiao", "noodle" }) );
        set_weight(50);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "一碗香喷喷冒着热气的面条,快喝,否则就凉了!\n");
                set("unit", "碗");
                set("value", 50);
...
{
        if( !arg || (arg != "mian tiao" && arg !="noodle"))
...
if (query("water_supply")) this_player()->add("water", query("water_supply"));
        ...
        if( !query("food_remaining") ) {
  message_vision("$N几口就把碗里的面条喝光了.\n",this_player());
        } else 
  message_vision("$N拿起碗来喝了一大口面条.\n",this_player());
        return 1;
}
