...
void create()
{
        ...
        if( clonep() )
                ...
                set("wield_msg",
                        HIC"天地间一道雳闪，青龙已出水，$n"+HIC"却在$N的手中，散发出摄人魂魄的光芒。\n"NOR);
                set("unequip_msg", HIM"$N将$n"+HIM"插回腰间的剑鞘。\n"NOR);
...
int wield()
{
       ...
       me = this_player();
       ob = this_object();
        if( this_player()->query_skill("tonggui-jian",1) < 200)
        {
                return notify_fail("你的同归剑法等级太低了，用不了这件兵器。\n");
        }
