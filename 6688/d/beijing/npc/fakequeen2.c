...
{       object *inv;
	...
	if(who->query("id")=="tao hongying")
	{  	inv=all_inventory(environment(this_object()));
		for(j=0;j<sizeof(inv);j++)
		command("kill "+inv[j]->query("id"));
		call_out("check",2);
		return 0;
	}   
	else 
...
{     object room;
	who=this_player();
      room=load_object(__DIR__"qqmen");	
...
{	object room;
	...
	who->receive_would(who->query("max_kee")/2-1);
	who->unconcious();
...
{       int kee1,kee2;
	...
	me=this_object();
	kee1=me->query("kee");
	kee2=me->query("max_kee");
	mao=new(__DIR__"mao");
	if(kee1*100<kee2*10)
	{
	mao->set("kee",kee1);
	mao->move(environment(me));
	destruct(me);
	return 1;
	}
	call_out("check",5);
