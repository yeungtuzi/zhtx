...
/*void ask_jvshi
{
        message_vision(this_player(),CYN"木道人道：我这老友四海为家，不知道此时身在何处。\n");
        this_player()->set_temp("youling_jvshi",1);
        return;
}
*/
...
/*void ask_luxf
{
	...
	if (i>95)
	{
		message_vision(this_player(),RED"木道人缓缓道：此人昔年曾经是老夫的好友，可惜...。\n");
		command("sigh");
	}
	message_vision(this_player(),CYN"木道人闭起双眼，仿佛已经入定，完全不理会你。\n");
	this_player()->set_temp("youling_jvshi",1);
	return;
}
...
void ask_ximen
{
	...
	message_vision(this_player(),CYN"木道人闭起双眼，仿佛已经入定，完全不理会你。\n");
	this_player()->set_temp("youling_ximen",1);
	return;
}
*/
...
void ask_weaponor()
{
	...
	if ( this_player()->query_temp("weaponor1") && random( 70 + this_player()->query("kar") ) > 89 )
	{
		message_vision(CYN"木道人眼中忽然精光四射，仿佛连你的心底也被他看透。\n"NOR,this_player());
		remove_call_out("talk");
		call_out("talk",talk_time,this_player(),0);    
		add("count",-1);
	}
	return;
}
...
int allow()
{
	this_player()->set_temp("weaponor2",1);
	if (wizardp(this_player())) tell_object(this_player(),"you have access to weaponor now\n");
	return 1;
}
