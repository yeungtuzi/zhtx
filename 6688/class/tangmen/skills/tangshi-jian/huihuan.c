...
int perform(object me, object target)
{
        ...
        if (!objectp(ob = present("huixuan biao",me) ) )
                return notify_fail("你身上没带着回环镖，如何使用「剑镖回环」？\n");  
        
        if( userp(me) && (int)me->query("force") < 2000 ) 
        ...
        me->set_temp("skill_mark/huihuan",1);
        ...
        message_vision(HIM"\n回旋镖"+HIC"欢快的发出一阵轻响，直接飞回到$N的手中。\n"NOR,me,target);
                }else if( target->query_skill("dodge")+target->query_skill("parry") < random((me->query_skill("sword") + me->query_skill("throwing"))*4/3))
        {
        message_vision(HIR"\n$n疲于应对$N的迎头三剑，那一点乌光却从$n的左臂对穿而出，一阵阵的麻痒让$n痛苦不迭！！\n"NOR,me,target);
        target->apply_condition("tm_poison",10 + target->query_condition("tm_poison") );     
...
int pfm2(object me, object target)
{
        ...
        message_vision(HIC"\n寒光一闪，"+HIM"回旋镖"+HIC"飞回$N的掌中，$N蓄势待发，酝酿着下一波的攻击。\n"NOR,me,target);

                }
                else
                {
        message_vision(HIM"\n回旋镖"+HIG"来势太急，$N一时手忙脚乱，竟然也没能接住，任由它飞了出去！\n"NOR,me,target);
                me->delete_temp("skill_mark/huihuan");
                }
        return 1;
...
int pfm3(object me, object target)
{
        ...
        message_vision(HIG"\n\n\n$N凝神而立，持镖在手，当$n全神贯注于$N的暗器之时，\n一把长剑却如毒蛇般悄无声息地袭来，且等$n有所察觉，一道寒光又飞向$n的面门！！\n"NOR,me,target);
        me->add("force", -500);
        if( target->query_skill("dodge")+target->query_skill("parry") < random((me->query_skill("sword") + me->query_skill("throwing"))*4/3))
        {
        message_vision(HIR"\n$n此时已心力交困，再无能力躲避，纵横江湖数十年，终于体会到了“人为刀俎，我为鱼肉”的感觉！！\n然而此刻已不容其多想，疼痛正在逐渐的消失，取而代之的是更为钻心的苦楚！！\n"NOR,me,target);
        target->apply_condition("tm_poison",10 + target->query_condition("tm_poison") );     
        target->update_condition();
        target->update_condition();
        COMBAT_D->report_status(me,target,1);   
        }else{  
        message_vision(HIY"\n$n疾往上纵，但体内的真气已渐渐不纯，只跃起八尺多高，\n好在$N旨在攻其下盘，这才险险避开！\n"NOR,me,target);    
        target->start_busy(1+random(2));
        }
        me->add_temp("apply/damage", num);
        me->add_temp("apply/attack", num);
        ...
        message_vision(HIM"\n回旋镖"+HIC"欢快的发出一阵轻响，直接飞回到$N的手中。\n"NOR,me,target);
                ...
                remove_call_out("pfm3");
                call_out("pfm2", 3, me, target);
        message_vision(HIM"\n回旋镖"+HIC"在空中划了一道美丽的弧线，仍然盘旋在$n的四周。\n"NOR,me,target);

                }
                else
                {
        message_vision(HIM"\n回旋镖"+HIG"好像摆脱了$n的控制，如脱缰的野马飞射了出去！\n"NOR,me,target);
                me->delete_temp("skill_mark/huihuan");
                }
        return 1;
