...
//     现在per和elo都很高，这个判定没用了，劝架的任务等于白送，改改公式
        if ( random(me->query("elo")) > 20 || random(me->query("per")) > 20 || random(5) == 1 )
        {
        ...
        else if ( me->query("elo") > 20 && random(3) < 1 && !me->is_fighting(ob) )
        {
                message_vision(CYN"$N说道：这是我们神教内部的事，别人不便插手！\n"NOR, ob);
                me->start_busy(3);
                return 1;
        }
        else if ( random(3) < 2 && !me->is_fighting(ob) )
        {
                message_vision(HIR"$N说到：你唠叨些什么？既然被你看见了，你就和这个叛徒一起去见阎老五吧！\n"NOR, ob);
                ob->kill_ob(me);
                if (random(obj->query("int")) < 20)
                {
