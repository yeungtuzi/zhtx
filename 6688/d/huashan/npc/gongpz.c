...
int do_group(string arg)
{
	...
	if(arg=="startrace")
	{
		...
		if(!wizardp(this_player())&&!raw_wiz_level(this_player())) return notify_fail("只有巫师才能开始。\n");
		command("chat 团体比赛开始。");
	...
	if( sscanf(arg, "add1 %s", arg)==1 )
	{
		...
		if(wiz_level(this_player()) == 0&&!raw_wiz_level(this_player())) return notify_fail("只有巫师才能加名额。\n");
...
int do_start(string arg)
{
	...
	if ( !wizardp(this_player())&&!raw_wiz_level(this_player()) ) return notify_fail("你无权举行比武！\n");
	over=0;
...
int do_queren()
{
	...
	if ( !wizardp(this_player())&&!raw_wiz_level(this_player()) ) return notify_fail("你无权确认比武！\n");
	if(!query("queren"))
...
int do_add(string arg)
{
	...
	if(!wizardp(this_player())&&!raw_wiz_level(this_player())) return notify_fail("只有巫师才能加名额。\n");
