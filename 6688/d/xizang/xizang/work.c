...
// here set normal physical work improvement to strength
        if ( temp>(my_str+1)*(my_str+1) ){
         my_str+=1;
         temp=0;
         message_vision("由于终日辛苦劳作,$N觉得自己变得更强壮了.\n",me);
         }
//        me->set("str", my_str);
        me->set("workpoints", temp);

// to improve "根骨", for outdoors work
        if ( (my_con <25) && (random(31) > 29)) {
         my_con+=1;
         message_vision("由于经常在室外工作,$N觉得自己的根骨得到了强化.\n",me);
//         me->set("con", my_con);
         }

// an accident, also a chance to improve courage
        if ( random(31) > 29 ) {
         message_vision("$N大叫一声:不好!从悬崖峭壁上摔了下来!!\n",me);
         skill = me->query("dodge", 1);

// lucky or dodge can avoid damage
         if ( random (my_kar+skill) <30 ) {
          message_vision("$N摔得头破血流!\n",me);
          me->add("kee", -30);
          } else {
             message_vision("$N真是好运气,居然没受什么伤.\n",me);
          }
         message_vision("$N大难不死,觉得自己的胆量有所提高.\n",me);
         my_cor += 1;
//         me->set("cor", my_cor);
         }

// assume using hammer in stone work
        me->improve_skill("hammer", random(my_str) +1 );
