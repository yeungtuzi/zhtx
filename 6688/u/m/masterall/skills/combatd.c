...
//   1.修改硬功的作用效果，被破之后扔可以减弱对自己的一部分伤害
//   2.硬功作用效果加倍
//   3.apply/dodge和parry的上限从30调整到50
//   2003.9.5 by masterall

//加入对attack，defense上限要求，apply/attack最大80,apply/defense最大20
//希望能有效防止溢出的问题。
//2003.11.4. by masterall
...
varargs int skill_power(object ob, string skill, int usage)
{
        ...
        int eff_level,afford, maxattack, maxdefense;
        string map_skill;

        maxattack = ob->query_temp("apply/attack");
        maxdefense= ob->query_temp("apply/defense");
                        if( maxattack > 80) maxattack = 80;
                        if( maxattack < 0) maxattack = 0;
                        if( maxdefense > 20) maxdefense = 20;
                        if( maxdefense < 0) maxdefense = 0;
	...
	//necromancy dec defense power
		...
		{   
			if( ob->query_skill_mapped("spells") == "necromancy" )
				power -= power /200  * ob->query_skill("necromancy",1); 
...
if( power < 0 )
	...
	if( ob->query_temp("ally/coefficient") )
		power *= ob->query_temp("ally/coefficient") / 100;
...
{
	return ( living(victim) && (me != victim) && (random(ap+dp)<dp) && (random(action["unparryable"])<10||victim->query_skill("parry")>350));
}

int is_couple(object ob1, object ob2)
{
	if( ob1->query("gender") != "男性" || ob2->query("gender") != "女性" )
        	if( ob2->query("gender") != "男性" || ob1->query("gender") != "女性" )
			 return 0;

	if( ob1->query("wife/id") == ob2->query("id") && ob2->query("husband/id")==ob1->query("id")) return 1;
	if( ob1->query("husband/id") == ob2->query("id") && ob2->query("wife/id")==ob1->query("id")) return 1;
	return 0;
}
...
object partner_can_parry(object me,object victim,int ap,mapping action,string attack_skill)
{
	...
	if( !arrayp(partners = victim->query_temp("ally/partner")) )
	{
		...
		coefficient = victim->query_temp("ally/coefficient");
	...
	for( i=0;i<sizeof(partners);i++ )
	{
		...
		coef = (int)SKILL_D(parry_skill)->query_sheng_ke(me->query_skill_mapped(attack_skill));
		if( coef > 100 )  pp = pp*coef/100;
		if( coef < -100 ) pp = pp*100/(-coef);
//		coefficient *= this_partner->query_lea()/20*victim->query_lea()/20;
		//加入夫妻感情指数的影响
		if( is_couple(this_partner,victim) )
		{
			c_faith = this_partner->query("couple_faith");
			if( !intp(victim->query("marry_times")) || victim->query("marry_times") <= 1 )
				victim->set("marry_times",1);
			c_faith /= victim->query("marry_times");
			coefficient += coefficient * c_faith / 50000;
		}	
		if( this_partner->is_busy() ) pp /= 3;
		if( pp < 1 ) pp = 1;
//		if( coefficient == 0 ) coefficient = (int)this_partner->query_temp("ally/coefficient");
		pp = pp/100*coefficient;
...
{
	int damage,damage_bonus,defense_factor,absorb_vic,bounce,myexp,bellicosity;
        int real,real_coff;
...
//膂力太高的也不合适，但是这样的NPC很多
	if( damage_bonus > 600 ) damage_bonus = 600;
	...
	//(canceled) 需要削弱一下现有的伤害力，太强了，战斗持续时间太短，相应的太极神功的防御系数下降
	damage = damage*80/100;
	damage_bonus = damage_bonus*80/100;
	...
	if( martial_skill = me->query_skill_mapped(attack_skill) )
	{
		if( martial_skill=="spring-blade" || martial_skill=="six-chaos-sword" || martial_skill=="celestrike" )
		{
			bellicosity = me->query("bellicosity");
                          //非fighter弟子杀气无效
			if(me->query("family/family_name") == "天邪派")
                           {
                          while( bellicosity > 1000 )
                          {
                                  damage = damage*12/11;
                                  bellicosity /= 5;
                          }       
...
//        硬功效果加倍，以前太弱了，by masterall
	...
	if( victim->query_skill("iron-cloth"))
		...
		{
		        if( SKILL_D(absorb_skill)->effective_level() )
		                absorb_vic *= SKILL_D(absorb_skill)->effective_level()/10;
	...
	if( (damage - absorb_vic) < 0 )
        {
                 absorb_skill = victim->query_skill_mapped("iron-cloth");
                 if( absorb_skill)
                                bounce = ((SKILL_D(absorb_skill)->bounce_ratio()) * (damage) / 10);
                 else
                                bounce = 0;
	...
	//硬功被打破了
	//		damage += 3*absorb_vic;
        //以前硬功被打破就不起防御效果了，我认为不大合理，还是要absorb一点伤害的
        //尤其现在攻击力整体都加强，硬壳的效果体现不出来了  by masterall
                        damage -= absorb_vic*3/2;
	...
	if( force_skill &&  foo = (int) SKILL_D(force_skill)->query_relieve_coff())
	{
		...
		if( foo >= 90 ) foo = 90;
                // 金钟罩卸力效果
                if ( force_skill == "hunyuan-yiqi")
                {
                // need mark
                if ( victim->query_temp("pfm/shaolin_jzz") )
                {
        ...
        //先天真气的作用“导”
        real = victim->query("realforce");
        if( real > 8000 ) real = 8000;
        if( victim->query_temp("real_guide") && real > 0 ) //need mark and have realforce
        {
                       victim->add("realforce", -1*damage/4);
                       damage -= damage * real/12000;    
                       message_combat(1,HIG"$N心神一动，先天真气携着$n灌注的无比真力如江河入海般导入$N的体内，瞬间便消声匿迹！\n"NOR, victim, me);
                       if (me->query("realforce") < 0)
                       {
                       me->set("realforce", 0);
                       me->delete_temp("real_guide");
                       message_combat(1,HIR"$N突然发觉自己体内的先天真气已然枯竭，再无从前的从前那种滔滔江水连绵不绝的气势了。\n"NOR, victim);
                       }
        }
...
varargs int do_attack(object me, object victim, object weapon, int attack_type,mapping attack_action,object controler)
{
	...
	//选择攻击动作
 	...
 	if( !objectp(me) || !objectp(victim) ) return 0;
        //白虹掌力的作用
        if( !objectp(weapon) )
        	action["unparryable"] = (int)action["unparryable"] + (int)me->query("unparryable");
	...
	else
	{
		...
		if( (parryer=partner_can_parry(me,victim,ap,action,attack_skill))!=victim || can_parry(me,victim,ap,pp,action) )
		{
			...
			if( parryer != victim )
			{
				if( is_couple(parryer,victim) )
					result = HIM"这时，只见"+parryer->name()+"奋不顾身替伴侣接下了所有的攻势！\n"NOR;
				else
			...
			damage = RESULT_PARRY;
                        if( !stringp(me->query_skill_mapped("force")) || !SKILL_D(me->query_skill_mapped("force"))->post_parry_attack(me, parryer) ) { 
				if( stringp(parryer->query_skill_mapped("force")) )
                                	SKILL_D(parryer->query_skill_mapped("force"))->post_parry_attack(parryer, me);
                        }
			if( !objectp(weapon) || !weapon->post_parry_attack(me, parryer) ) {
				if( objectp(parryer->query_temp("weapon")) )
					parryer->query_temp("weapon")->post_parry_attack(victim, me);
			}
			result = "";
		}	
		// 命中,计算效果
		else
		{
			...
			damage = calculate_damage(me,victim,action,weapon,attack_skill);
                        ...
                        // limit the mirror shadow damage 
                        if(me->query("mirror_shadow"))
                        {
                                if(userp(victim)) 
                                        damage = damage/5;
                                else 
		...
		//武器有毒？
        	if (weapon && weapon->query("poison"))
        	{
	   		...
	   		if ( foo > 10 ) 
	   		{
                ...
                //先天真气 “破”
                real = me->query("realforce");
                if( real > 8000 ) real = 8000;
                real_damage =   real / 10;
                if ( real_damage < 1 ) real_damage = 1;
                if ( real_damage > 800 ) real_damage = 800;
                if( me->query_temp("real_break") && real > real_damage ) //need mark and have realforce
                {
                       me->add("realforce", -real_damage/2);
                       victim->receive_wound("kee", real_damage);
                       victim->receive_damage("kee", real_damage);
                       victim->add("force", -4*real_damage);
                       message_combat(1,HIR"$N感到$n的先天真气贯体而入，带动着自己体内的真气四处乱窜，简直是苦不堪言！\n"NOR, victim, me);
                }
        }
	//后续函数
...
int fight(object me, object victim)
{
	...
	if( victim->is_busy() || !living(victim) ) {
		...
		return 1;
		} else if( random( 2*max((int)victim->query_cps(),-20) + (int)victim->query("cps") ) < random(2*max((int)me->query_cor(),10) + 2*(int)me->query("cor")+(int)me->query("bellicosity") / 500) ) {
		me->set_temp("guarding", 0);
		if( !victim->is_fighting(me) ) victim->fight_ob(me);
		if( !me->visible(victim) ) message_vision(per_msg,me,victim);
...
void start_berserk(object me, object obj)
{
	...
	message_vision("$N用一种异样的眼神扫视著在场的每一个人。\n", me);
          ...
          if( me->query("taskkiller") ) return;
	if(	(int)me->query("force")/5 > (random(bellicosity) + bellicosity)/2 ) return;

         if( random(bellicosity) < (obj->query("per")-10)*(obj->query("per")-10)*20 )
	{
...
void start_hatred(object me, object obj)
{
	...
	message_vision( query_hunt_msg(), me, obj);	
         if(  random(me->query("combat_exp")) < (obj->query("per")-10) * (obj->query("per")-10) * 10000 )
	{
		message_vision("$N看了看$n，心一软，停手罢斗了。\n",me,obj);
		return;
	}
...
void start_vendetta(object me, object obj)
{
	...
	// We found our vendetta opponent! Charge!
        if( random(me->query("bellicosity")) < (obj->query("per")-10)*(obj->query("per")-10)*20 )
	{
		message_vision("$N看了看$n，心一软，停手罢斗了。\n",me,obj);
		return;
	}
...
void start_aggressive(object me, object obj)
{
	...
	||	environment(me)->query("no_fight") 	// Are we in a peace room?
          ...
          if( random(me->query("bellicosity")+10000) < (obj->query("per")-10)*(obj->query("per")-10)*20 )
      {
                  message_vision("$N看了看$n，心一软，停手罢斗了。\n",me,obj);
                  return;
        }
...
*/
		...
		if( (int)victim->query("potential") > (int)victim->query("learned_points"))
			victim->add("potential",
				((int)victim->query("learned_points") - (int)victim->query("potential"))/1 );

//		victim->skill_death_penalty(pk_flag);
		victim->new_death_penalty();
		bls = 10;
...
// for pkquest
 ...
 &&  userp(victim) )
 {
	...
	environment(killer)->add("resource/apparition", 1);
        ...
        if (userp(killer) && (killer->query("combat_exp") < victim->query("combat_exp")) && (killer->query("shen")< 2100000000)){
     	//added by yeung, to give the only killer reward of shen.
	//	if( victim->query_temp("beat"+killer->query("id")) && sizeof(victim->query_temp("beat"))==1 )
                        killer->add("shen", -(int)victim->query("shen") / 10);
                        if(killer->query("shen")> 2100000000) killer->set("shen", 2100000000);;
       }

	if( stringp(vmark = victim->query("vendetta_mark")) )
		killer->add("vendetta/" + vmark, 1);
        if((int)killer->query_temp("skill_mark/banruo-zhang"))
                killer->delete_temp("skill_mark/banruo-zhang");
}
...
{
        mapping quest;
        int mkee = victim->query("max_kee");
        int mgin = victim->query("max_gin");
        int msen = victim->query("max_sen");
 		...
 		&& (!me->query("over_time")))
 	{
        ...
        // add for cttask by dongsw
        if ( me->query("cttask/var") == "战胜" 
                        && interactive(me)
                        && !userp(victim)
                        && victim->query("var") == "战胜"
                        && victim->query("owner") == me )
        {

                        if ( !living(victim)
                                || (int)victim->query("kee") < 1
                                || (int)victim->query("sen") < 1
                                || (int)victim->query("gin") < 1 )
                                return 1;
                        if ( me->query_temp("cttask/win_time") == 1)
                        {
                                write("\n" + CYN + victim->name()+"说道：天外有天，人外有人，在下从此退隐江湖，闭门修炼。\n\n"NOR);
                                me->set("cttask/end_time", time());
                                me->delete_temp("cttask/win_time");
                                me->delete("cttask/get");
                                me->set("cttask/done", 1);
                		call_out("destruct_killer", 1, victim);
                                  return 1;
                        }
                        else 
                        {
                                  write("\n" + CYN + victim->name()+"说道：这次不服，如果你再能胜我，我才认输。\n\n"NOR);
                                victim->set("kee",mkee);
                                victim->set("eff_kee",mkee);
                                victim->set("sen",msen);
                                victim->set("eff_sen",msen);
                                victim->set("gin",mgin);
                                victim->set("eff_gin",mgin);
                                victim->set("force",victim->query("max_force"));
                                  write( HIG + victim->name()+"迅速的从衣兜里掏出“仙豆”放进嘴里，整个人好象复活了一样！\n"NOR);
                                  me->set_temp("cttask/win_time", 1);
                        return 1;
                        }
        }
...
void destruct_killer(object killer)
{ 
        write(YEL + killer->name()+"一个跟头，不见了人影。\n\n"NOR);
        destruct(killer);
}
