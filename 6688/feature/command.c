...
protected nomask int command_hook(string arg)
{
	...
	if( !query_heart_beat() )
	{
		...
		{
			this_object()->reset();
                	if(!this_object()->raw_set_heart_beat(1))
			{
				log_file("heart_bug",base_name(this_object()));
		...
		else
		{
			...
			tell_object(this_object(),"由于某种原因，系统错误导致你失去了心跳，请与巫师联系。\n");
			this_object()->delete_all_trigger();
			this_object()->reset();
			this_object()->raw_set_heart_beat(1);
//			return 1;
		}
	}
