...
varargs void improve_skill(string skill, int amount, int weak_mode)
{
	...
	//左手的人学起来比较费力...
	if( this_object()->query("fav") < 10 && this_object()->query("fav") > 1 )
                amount = amount / 2;

	// Give learning penalty to those learning too much skills.
	spi = query("spi");
	i = sizeof(learned) - spi;

	if( (i > 0) && (int)this_object()->query("env/message_brief") < 2 )
		tell_object(this_object(),"你只觉得脑筋有些发胀，贪多嚼不烂，要小心啊。\n");
