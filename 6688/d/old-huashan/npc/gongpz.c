...
int do_set(string arg)
{
	if(stringp(arg) && strsrch(arg,"attack") != -1)
	{
		write("这里不能调整攻防比.\n");
		return 1;
	}
...
int do_group(string arg)
{
	...
	if(arg=="startrace")
	{
		...
		if(wiz_level(this_player()) == 0) return notify_fail("只有巫师才能开始。\n");
		command("chat 团体比赛开始。");
		for(i=0;i<sizeof(group1);i++)
		{
			...
			for(j=0;j<sizeof(group2);j++)
			{
				...
				ob1->set("env/attack",50);
				ob2->set("env/attack",50);
				ob1->bihua_ob(ob2);
				ob2->bihua_ob(ob1);
			}
		}
...
int do_start(string arg)
{
	...
	if ( wiz_level(this_player()) == 0 ) return notify_fail("你无权举行比武！\n");
	over=0;
...
int do_queren()
{
	...
	if ( wiz_level(this_player()) == 0 ) return notify_fail("你无权确认比武！\n");
	if(!query("queren"))
...
int do_qu(string arg)
{
	...
	if(arg=="sword")
	{
		...
		{
			ob->move(me);
			if ( !wiz_level(me) ) me->set_temp("get_weapon",1);
...
int do_halt()
{
	...
	if( query("group_race") && !wiz_level(me))
	{
