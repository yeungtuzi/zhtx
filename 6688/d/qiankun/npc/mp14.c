...
HIC"“苍天不负我命，终于让我等到了这一天，好孩子，这把妖刀狗屠今后就交给你了！”\n",NOR,
HIR"“一定要好好珍惜它，记住，‘剑在人在，剑亡人亡！’”\n",NOR,
HIG"“听说我的师兄天骄后来归隐去了侠客岛，如果有机会的话将来你去找他，”\n",NOR,
...
void create()
{
        ...
        create_family("天邪派", 2, "无名前辈");
        set("title", HIB"乾坤教囚徒"NOR);
        set("nickname",HIR "逝去的辉煌，永世的神伤"NOR);
        setup();

        carry_object("/d/qiankun/obj/weapon/yaodao-goutu")->wield();
}
...
void die()
{
       ...
       command("sigh");
       if(me->query_skill("spring-blade",1) > 219) 
       {
       me->remove_all_enemy();
       ob->remove_all_enemy();
        revive();
...
{        
        object weapon = new("/d/qiankun/obj/weapon/yaodao-goutu");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("yaodao goutu",who) )) 
                {
                weapon->move(who);
                }
