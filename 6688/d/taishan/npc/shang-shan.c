...
void create()
{
	...
	set("weapon", "/d/taishan/obj/changjian");
	set("armor", "/d/taishan/obj/cloth");

	setup();

	carry_object("/d/taishan/obj/changjian")->wield();
	carry_object("/d/taishan/obj/cloth")->wear();        
	}
	else {
		set("id", "shangshan");
...
int do_kill()
{
	...
	message_vision("四周的红衣武士群起对$N发动攻击！\n", this_player());
        ...
        me->set("bellicosity",800);
        me->set("env/wimpy",0);


        command("exert poerup");
        command("cast astral vision");
...
int accept_fight(object ob)
{
	...
	me->set("force",  me->query("max_force"));
        me->set("mana",me->query("max_mana"));
        me->set("atman",me->query("max_atman"));
        me->set("bellicosity", me->query("max_force"));
        me->set("env/wimpy",0);


	if( me->query_skill("perception") < 100) 
		me->set_skill("perception",100);
...
int do_recopy(object me, object ob)
{
	...
	if ( me->query("winner") != ob->query("id") ) 
        ...
        if ( me->query("MagicNum") != 0
                   && me->query("MagicNum") != ob->query("MagicNum") )
                 return notify_fail("你想偷梁换柱,呵呵，没门\n");
...
/* copy entire dbase values */
	...
	hp_status = ob->query_entire_dbase();
		...
		me->set("combat_exp",hp_status["combat_exp"]);
                ...
                me->set("MagicNum", ob->query("MagicNum"));
	save();

	tell_object(ob, "状态储存完毕。\n");

	newob = new("shang-shan");
	if( newob) newob->move("/d/taishan/xiayi");
	destruct(me);
