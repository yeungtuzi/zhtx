...
/*        if(combatexp >=3686800) {
                tell_object(me,this_object()->query("name")+ "说道：你现在的武功已经青出于蓝而胜于蓝了。我没有什么任务给你了，你自己出去闯荡江湖吧！\n" NOR);
                return(1);
        }*/
...
//      depend the target's shen master may abort and reselect
        ...
        for( j=0;j<5;j++) {
                ...
                printf("quest = %s\nquest_type = %s\nquest_bonus =%d\nshen_type = %d",
                	quest["quest"],quest["quest_type"],quest["exp_bonus"],quest["shen_type"],);
...
//   always 12 min  
	...
	if (quest["quest_type"] == "打探")
        {
        tell_object(me,"替我打探『"+quest["quest"]+"』的消息。\n" NOR);
