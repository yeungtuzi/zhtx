...
void create()
{
	set("short", WHT"天池"NOR);
	set("long", @LONG
天池中满是冰块，一头白鳞冰龙趁机扑了过来。
LONG
	);
	set("exits",([
...
int valid_leave(object who,string dir)
{
	...
	if( objectp(dragon = present("ice dragon",environment(who) ) ) 
&& dir=="out" )
...
int sinking(object ob)
{
	int mykee,mycps,damage;
		...
		return 0;
	mycps=ob->query("cps");
	message_vision(HIW"$N感觉越来越难受, 身体冷得难受。\n", ob);
	message_vision(RED"$N不时被涌来的冰块呛进口中！\n"NOR, ob);
...
int no_quit()
{
        ...
        {
		message_vision("$N想溜之大吉，但困在冰块中，哪里作得了主。\n", ob);
                return 0;
        }
        return 1;
}
