...
int do_repair(string arg)
{
         ...
         if( !arg) 
                 return notify_fail("你修理什么东西?\n");   
         if( !objectp(ob = present(arg, me)) )
                 return notify_fail("你身上没有这样东西!\n");

/*
        if( ob->query("name") != ("断掉的" + ob->query("name")) )
                return notify_fail("你的这件东西没有坏啊？!\n");
*/

/*
                 if( ob->querry("weapon_prop") != 0 );
                 return notify_fail("你的这件东西没有坏啊？\n");
*/

          ob->set("name",(int)ob->query("name"));
            ob->set("weapon_prop",(int)ob->query("weapon_prop"));
         write ("你的兵器修理好了。\n"); 
         return 1;
}
