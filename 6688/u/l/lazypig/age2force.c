...
int main(object me, string arg)
{
       ...
       max_force = 3 * me->query_skill("force",1) + 5 * ( me->query_skill("force",0) + m
e->query_skill("force",1) / 2);
       start_force = me->query("max_force") ;

       if( start_force >= max_force ) return notify_fail("你已经到内力瓶颈了，修炼对你没
有任何帮助！\n");
       ...
       {
             if( me->query_skill("force") < 1) return notify_fail("你内功心法等级太低，
还不能长时间修炼。\n");
...
//     14 year old has the highest recover efficiency,heart beat is 4 times, 15-23 is 2
times, > 23 is 1
             ...
             eff_kee_2_force  = 10 * (me->query("max_kee")/25 + 1) ;   // 1 food = 10 he
art beat
...
//    1 heart-beat can transfer x kee to force: x = query("max_kee")/25 + 1
             ...
             auto_recover = 10 * ( start_force/10 + 2 * me->query("con") ) * eff_recover
 ;   // here, should be me->query("force") + 2*con) * eff_recover,
                                                                                  // but
 here we just use the average force. lazypig
...
heart_point1 = 1 + ( auto_recover - eff_kee_2_force ) / auto_ex_time  ;

heart_point2 = 1 + ( eff_force_2_kee - eff_kee_2_force ) / loop_ex_time ;

//    check if one can exercise with looping.

             if ( heart_point1 < heart_point2 ) heart_point1 = heart_point2;

                      total_point = (int) heart_point1 * t_hours * 3600 * eff_recover / 25 ; // how much total
                              // force points one can get in 1 hour

/*    calculate the max_force point one can get
         start max_force is N, final max_force is M, then total_point = ( M - N + 1 ) *
( M + N ) / 2
         -------------------------------> M = (int) (sqrt( 1 + 4 * ( 2 * total_point + N
 * N - N) ) - 1) /2  ~ sqrt( 2* total_point + N * N - N)

   But it seams there is no sqrt in LPC ?  so use a approx value for M ~ N + total_point
 / N - total_point / N / 6             */

//             final_force = (int) (sqrt ( 1 + 4 * ( 2 * total_point + start_force * sta
rt_force - start_force ) ) - 1) / 2 ;

             final_force = start_force + 10 * total_point / start_force / 11 ;

            if ( start_force < 1000 ) final_force = start_force + 7*total_point / start_force / 8 ;

             if ( final_force > max_force ) final_force = max_force ;    // it shouldn't                                    be more than bottle net

             me->set("max_force",final_force);

             printf(HIC"经过%d小时的修炼，你的内力提升了%d点\n"NOR,t_hours,final_force-s                                   tart_force);
             return 1;
         }
        return 0;
}
