...
void create()
{
        ...
        set("long",@LONG
他容貌清癯，颏下疏疏朗朗一丛花白长须，垂在胸前，手持
酒杯，眼望远处黄土大地和青天相接之所，对围着他的众人竟正
眼也不瞧上一眼。他背上负着一个包袱，再看他腰间时，却无弯
刀。原来他竟连兵刃也未携带。LONG);
        set("gender", "男性");
        set("age", 55);
...
{ 
	...
	{
	  command("? "+me->name());
	  command("say 这位"+RANK_D->respect(me)+"，无缘无故问我这个干什么？\n");
	  return 0;
	};
	if (me->query("combat_exp")<1000000)
	{
	  ...
	  command("say 这位"+RANK_D->respect(me)+"你功夫不够，没法帮助老夫。\n");
      return 0;
	};
	else
	{
    ...
    command("say 既然这位"+RANK_D->respect(me)+"愿意襄助老夫，老夫不胜感激。\n");
