...
{
	return __DIR__"data/cell";
}

void create()
{
	set_name( "流行病毒", ({ "cell", "liuxing bingdu", "bingdu" }) );
	if( !restore() ) {
		set("short", HIC "可怕的流行病毒" NOR "(cell)");
...
void greeting()
{
	...
	obbody=ob->query("str")*1/2+ob->query("con")*1/2-ob->query("age")*1/2;
	mybody=me->query("str")*1/4+me->query("con")*1/4+random(20);

	if( !ob || environment(ob) != environment() ||
obbody>mybody || random(100)<60) 
	{
		command("whisper "+ob->query("id") + " 算你运气好！\n");
			return;
	}
...
void use_poison()
{
	...
	obbody=ob->query("str")*1/2+ob->query("con")*1/2-ob->query("age")*1/4;
	mybody=me->query("str")*1/2+me->query("con")*1/2+random(20);
	if( ob->query_condition("cell_poison") ) return;

	tell_object(ob, "你突然觉得体内侵入了一种病毒，有种不快的感觉。\n");
	if( obbody<=mybody && random(100)>=60 )
		ob->apply_condition("cell_poison", 20);
}
...
//These are for commonly combating.
