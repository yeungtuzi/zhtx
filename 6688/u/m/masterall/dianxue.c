...
// 对npc赚一点
        ...
        if( random(ap+dp) > dp ) {
                ...
                return notify_fail(HIY对方手中都没兵刃了，点他作甚？\nNOR)
                ...
                target->add(eff_kee,-((int)me->query(max_kee)/4))
                target->add(eff_gin,-((int)me->query(max_gin)/4))
                target->add(eff_sen,-((int)me->query(max_sen)/4))
                target->start_busy(2)
                  }
...
void remove_effect(object me,object target,int amount)
{
                ...
                tell_object(me, target->name() +HIW潜用内力撞开了被封的穴道，顿时行动恢复正常...\n NOR)
