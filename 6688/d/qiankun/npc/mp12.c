...
HIC"“苍天不负我命，终于让我等到了这一天，好孩子，这把七彩宝石戒指今后就交给你了！”\n",NOR,
HIR"“一定要好好珍惜它，记住，‘剑在人在，剑亡人亡！’”\n",NOR,
HIG"“听说我的师姐云韵后来归隐去了侠客岛，如果有机会的话将来你去找他，”\n",NOR,
...
void create()
{
        ...
        create_family("逍遥派", 2, "无名前辈");
        set("title", HIB"乾坤教囚徒"NOR);
        set("nickname",HIR "逝去的辉煌，永世的神伤"NOR);
        setup();

        carry_object("/d/qiankun/obj/weapon/xiaoyao-ring")->wield();
}
...
{        
        object weapon = new("/d/qiankun/obj/weapon/xiaoyao-ring");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("xiaoyao ring",who) )) 
                {
                weapon->move(who);
                }
