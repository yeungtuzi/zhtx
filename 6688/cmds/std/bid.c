...
int main(object me, string arg)
{
	...
	if( !(room=find_object("/d/jingcheng/tamgch")) )
		room= clone_object("/d/jingcheng/tamgch");
	
	if( !(obj=present("paimai shi",room)) )
	{
		obj=new("obj/npc/paimaishi");
		obj->move("/d/jingcheng/tamgch");

		return notify_fail("拍卖师现在在京城广场，但没有在拍卖。\n");
	}
	else
	{
	...
	//yeung
	//to denine biding 2100000000
	// 99/03/26
	if( argg >= 1000000)
	{
		return notify_fail("你哪里来的这么多钱?想哄抬物价吗?\n");
	}
	...
	obj->report_paimai(me,argg,obj->query("num"));
