...
string ask_for_join()
{
 ...
 if( (int)me->query("cps")<30 ) 
   {
              command ("say 施主定力太低，一旦入我佛门，受不了清规戒律之苦的，还是请回吧！");
             return;
   }
...
void greeting(object ob)
{
        ...
        if( ob->query("family/family_name") =="少林派" && ob->query_condition("drunk"))
        {
        message("system",HIR"【纵横天下—少林寺】清法比丘(Qingfa biqiu)："+ob->name()+"这个不争气的弟子，居然胆敢破了酒戒！\n将"+ob->name()+"打入僧监拘押三月，非洗心悔改，不得释放！意图越狱者罪加一等！！\n"NOR,users() );
