...
string ask_me()
{
	mapping fam, my_fam, skl; 
        object fighter, me, room, monk,*monkteam=({});
	string *sname;
	int i,j;
        ...
        for(j=1; j<10; j++) 
	{
		...
		if( !objectp(monk = present(names[j-1], room) ))
		{
	        	me->move("/d/shaolin/banruo5");
        	        message_vision("\n玄难大师走了过来,对$N说: 真是对不起，般若堂中有人不在，无法举行罗汉大阵。\n\n", fighter);
			return 0;
		}
...
//		room->delete("exits");
		fighter->set_temp("luohan_passing", 1);
	}

	for(i=0;i<sizeof(monkteam);i++)
	{
                message("vision", monkteam[i]->query("name") + "罗汉点了点头，快步走了出去。\n", environment(monkteam[i]),monkteam[i] );
                monkteam[i]->move("/d/shaolin/wuchang1");
	}
...
int waiting(object me)
{
	...
	if( wait_time == 300 )
	{
		...
		find_player(me->query("assigned_fighter"))->delete_temp("xuannan-asked");
...
int preparing(object me, object fighter)
{
	...
	message("vision", HIY "\n玄难大师大声宣布：西罗汉大阵合围！\n\n" NOR, room);
        command("chat " + fighter->query("family/family_name") + "弟子" + fighter->query("name") +
...
//	message_vision(HIY "xuannan's count = " + count + "\n" NOR, fighter);
	...
	!living(fighter) || fighter->query("kee") <= 1 )
	{
		if( objectp(present(me->query("assigned_fighter"), environment(me)) )) {
			fighter->delete_temp("luohan_passing");
			fighter->delete_temp("beat_count");
		}
		monk1->move(room1);
