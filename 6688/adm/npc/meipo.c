...
//		return notify_fail("真是抱歉，请您等一下到红娘庄来找我吧。\n");
	...
	if( !((object)obj->query_temp("um_"+me->query("id"))) )
	{     
		inv=all_inventory(me);
		i=sizeof(inv);
		while(i--)
		{
			if( me->query_temp("um_"+inv[i]->query("id")) ) 
			{
				tell_object(inv[i], YEL + me->name() + 
				"取消了和你离婚的念头。\n" NOR);
				me->delete_temp("um_"+obj->query("id"));
				break;
			}
		}
		...
		message_vision(MAG "\n$N对著$n说道：" 
			...
			+ me->name() + "，咱们解除婚约吧!好吗?\n\n"
			NOR, me, obj);           
		me->set_temp("um_"+obj->query("id"), 1);
	...
	obj->delete_temp("um_"+me->query("id"));	                                                                           
	...
	str1 = me->query("room/id");
	if ( str1 ) 	rm("/data/room/"+str1+".c");
