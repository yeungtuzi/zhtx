...
void check_trigger()
{
    ...
    if(!query("exits/enter") ) {
        if( !(room = find_object(__DIR__"jsduchuan")) )
            room = load_object(__DIR__"jsduchuan");
        if( room = find_object(__DIR__"jsduchuan") ) {
            if((int)room->query("yell_trigger")==0 ) {
                room->set("yell_trigger", 1);
                set("exits/enter", __DIR__"jsduchuan");
...
void arrive()
{
    ...
    if( room = find_object(__DIR__"jsduchuan") )
    {
        room->set("exits/out", __DIR__"jsdukoud");
