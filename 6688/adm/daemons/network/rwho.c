...
string remote_who(string mud, object source)
{
  ...
  (AUX_PATH + "rwho_q")->send_rwho_q(mud, source, 0);
  ...
  if(sizeof(whos))
    {
      ...
      who = sprintf("%s (%s driver, %s mudlib)\n(# = wiz)  %66-=s\n",
		    Mud_name(), __VERSION__, MUDLIB_VERSION, who);
    }
  else
    who = sprintf("%s (%s driver, %s mudlib)\n--Nobody logged on--\n",
		  Mud_name(), __VERSION__, MUDLIB_VERSION);
...
get_name(object obj)
{
  return (wizardp(obj) ? "#" : "") + capitalize(geteuid(obj));
}
