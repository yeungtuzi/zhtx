...
//        printf("dodge_points:%d\n",dodge_points);
          ...
          if(weapon)
             real_attacks = attack_points/100 + parry_points/100 + 1;
          else
             real_attacks = attack_points/100 + parry_points/200 + 1; 
...
/* name, family, exp, shen, */
          ...
          npc_items=sprintf("%s,%s,%10d,%10d,npc,%O,npc\n",npc->query("name"),fam_name ,real_attacks,npc->query("shen"),npc); 
          write_file(listfile+".npc",npc_items,0);
...
//        printf("%s 有 %d 个物品\n",npc->query("name"),sizeof(inv));
        for(i=0;i<sizeof(inv);i++){
            ob_items=sprintf("%s,%s,%10d,%d,%s,%O,obj\n",inv[i]->query("name"),fam_name,real_attacks,npc->query("shen"),npc->query("name"),inv[i]);
