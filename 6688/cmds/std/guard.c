...
int main(object me, string arg)
{
	...
	if( arg=="master")
	{
		...
		{
			if( env[i]->is_fighting()
				&& env[i]->query_temp("killmaster")
 				&& env[i]->query("family/family_name")
				&& env[i]->query("family/family_name")==me->query("family/family_name") )
			{
				killer = env[i]->query_temp("killmaster/killer");
				for(j=0;j<sizeof(killer);j++)
				{
					if(ob=present(killer[j],environment(me))) enemy += ({ob});
				}
			}
		}
		if( ! sizeof(enemy)) return notify_fail("你的师门现在并不需要保护。\n");
		if(member_array(me,enemy)!=-1)
			return notify_fail("你要自己打自己吗?\n");

		message_vision(HIW"$N加入战团以维护师门尊严。\n"NOR,me);

		env=users();
		for(i=0;i<sizeof(env);i++)
		{
			if(env[i]==me) continue;
			if( env[i]->query("family/family_name")==me->query("family/family_name"))
				obj += ({env[i]});
		}
		obj += ({me});
...
message_vision(RED"$N叹了口气,说道:你我无往日无冤,今日无仇,今天师门有难
,不得不然,得罪了!\n"NOR,me);
		for(i=0;i<sizeof(enemy);i++)
		{
			enemy[i]->fight_ob(me);
tell_object(enemy[i],HIR"看起来"+me->name()+"想杀死你!!!\n"NOR);
			me->bihua_ob(enemy[i]);
		}
		me->set_temp("guarded_master",1);
		return 1;
	}
