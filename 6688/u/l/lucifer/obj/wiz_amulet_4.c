    ...
    int i, all, player_gil_flag, heal_kee, maxkee, effkee, kee;
        ...
        {       
                if( me->is_fighting() && (gil) && !me->query_condition("zhaohuan_weaponor" ) )
                {
                        //只在战斗之初判断一次是否召唤基加美修
                        gil = 0;
                        if( random(100)>0 && random(player_gil_flag) > 0)
                        {       
								...
								me->apply_condition("zhaohuan_weaponor", 5+random(10) );
                ...
                if( !living(me) && (wake) )
                {
                        ...
                        if( random(player_gil_flag) > 1500)
                        {
							    ...
							    if ( bonus && !environment(me)->query("no_fight") && present(victim, environment(me) )  && !(victim==me) )
								{
...
int do_feel(string arg)
{
	...
	int value = 30 * (player_gil_flag - 450) /  player_gil_flag;
...
int get()
{
	if( !this_player()->query("bingqipu_success") )
	{
		tell_object(this_player(),"不是你的东西，乱拣什么！“路不拾遗”，这句话没听过嗦！\n");
		return 0;
	}
	return 1;
}
...
int bonus_damage(object me, object victim, int bonus)
{
        ...
        if(userp(victim))
        {
			...
			COMBAT_D->report_status(me,victim);
