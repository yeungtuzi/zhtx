...
{
        int lvl;
        lvl=me->query_skill("tonggui-jian", 1);
        if(damage <= 1)return;  
        ...
        if( damage > 0 )
        {
                if( objectp(me) )
                {
                        ...
                        me->receive_wound("kee",damage*180*180/(4*lvl*lvl));
...
void post_action2(object me,object victim,object weapon,int damage)
{
        ...
        if( damage > 0 )
        {
                if( objectp(me) )
                {
                        ...
                        me->receive_wound("kee",damage*180*180/(3*lvl*lvl));
                        COMBAT_D->report_status(victim,me,1);
                        message_vision("\n\n",me);                        
                }
        }
...
void post_action3(object me,object victim,object weapon,int damage)
{
        ...
        if( damage > 0 )
        {
                if( objectp(me) )
                {
                        ...
                        me->receive_wound("kee",damage*180*180/(2*lvl*lvl));
                        COMBAT_D->report_status(victim,me,1);
                        message_vision("\n\n",me);                        
                }
        }
...
{
        int skill,extra,busy,i,per_power,per_lev,victim_exp;
        object ob;
        if( !me->is_fighting() )
