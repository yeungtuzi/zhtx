...
int wield()
{
       ...
       me = this_player();
       ob = this_object();
        if( this_player()->query_skill("mantian-huayu",1) < 200)
        {
                return notify_fail("你的满天花雨等级太低了，用不了这件兵器。\n");
        }
        if( this_player()->query("combat_exp") < 5000000)
        {
                return notify_fail("你江湖经验太低了，不配拥有这件举世无双的暗器。\n");
        }
        return ::wield();
}
