...
{
	object cloth;
	int orig_gin,orig_kee,orig_sen;
	...
	orig_sen=ob->query("eff_sen");
	if(orig_sen < 5 ) orig_sen = 5;
	orig_kee=ob->query("eff_kee");
	if(orig_kee < 5 ) orig_kee = 5;
	orig_gin=ob->query("eff_gin");
	if(orig_gin < 5 ) orig_gin = 5;

	ob->reincarnate();
        ob->set("eff_gin", orig_gin);
        ob->set("eff_kee", orig_kee);
        ob->set("eff_sen", orig_sen);
