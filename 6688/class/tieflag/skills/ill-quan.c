...
mapping enable_req() {
        ...
        if( this_player()->query_skill_mapped("force") == "yunhai-force" ){
        return( ([ 
                "force":"yunhai-force",     
        ]) );
        }else if( this_player()->query_skill_mapped("force") == "jiayiforce" ){
        return( ([ 
                "force":"jiayiforce",   
        ]) );
        }else{
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if( !inv || !sizeof(inv)) return damage_bonus;
	dress=inv[random(sizeof(inv))];
	if( !objectp(dress) ) return damage_bonus*2;
    	if( (string)dress->query("equipped")=="worn" )
        if (dress->unequip()) {
             if( !stringp(dress->query("unequip_msg")) )
                message_vision(YEL "$N将$n脱了下来。\n" NOR,me,dress);
             else
                message_vision(dress->query("unequip_msg"),me,dress);
	    	dress->move(environment(me));
	     return damage_bonus*2;
        };
	return damage_bonus*3/2;
}
