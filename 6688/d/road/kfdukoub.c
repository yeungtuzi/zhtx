...
LONG
    ...
    set("exits", ([
                   "north" : "/d/road/rdktojd0",
    ]));

    set("item_desc", ([
        "river" : "近岸处有一叶小舟和一只快船，也许喊(yell)一声船家就能听见。\n",
    ]));
...
void check_trigger()
{
    ...
    if(!query("exits/enter") ) {
        if( !(room = find_object(__DIR__"kfduchuan")) )
            room = load_object(__DIR__"kfduchuan");
        if( room = find_object(__DIR__"kfduchuan") ) {
            if((int)room->query("yell_trigger")==0 ) {
                room->set("yell_trigger", 1);
                set("exits/enter", __DIR__"kfduchuan");
...
void check_trigger_fast()
{
    ...
    if(!query("exits/fastboat") ) {
        if( !(room = find_object(__DIR__"kfduchuan_fast")) )
            room = load_object(__DIR__"kfduchuan_fast");
        if( room = find_object(__DIR__"kfduchuan_fast") ) {
            if((int)room->query("yell_trigger_fast")==0 ) {
                room->set("yell_trigger_fast", 1);
                set("exits/fastboat", __DIR__"kfduchuan_fast");
                room->set("exits/out", __FILE__);
                message("vision", "一叶扁舟缓缓地驶了过来，艄公将一块踏脚"
                    "板搭上堤岸，以便乘客\n上下。\n", this_object() );
                message("vision", "艄公将一块踏脚板搭上堤岸，形成一个向上"
...
void on_board()
{
    ...
    message("vision", "艄公把踏脚板收了起来，竹篙一点，扁舟飞快的向江心驶去。\n",
        this_object() );
...
void arrive()
{
    ...
    if( room = find_object(__DIR__"kfduchuan") )
    {
        room->set("exits/out", __DIR__"kfdukoun.c");
