// kickout.c
// as suggestion from yuer ,only admin can kickout
#include <ansi.h>
#include <command.h>
inherit F_CLEAN_UP;
...
/*                  if (    !living(ob)
	...
	log_file("KICKOUT_PLAYER",sprintf("%s(%s) kickouts %s(%s) on %s and denys his relogin for %d minutes.\n",
		me->name(1), geteuid(me), ob->name(1), geteuid(ob),ctime(time()),block_time ) );
        ob->save();
        destruct(ob);
        seteuid(getuid());
        return 1;
