...
void create()
{
	set_name(WHT"兵器贩子"NOR, ({ "weaponor","bingqi fanzi"}) );
	set("long",BLK@LONG
...
void slash(object victim, object player)
{
		...
		switch (s)
		{
			...
			case 2: sword_name=BLK"斩铁剑"NOR;
					damage = 2000 + random(1500); 
					if (wizardp(player)) tell_object(player,"dam is "+damage+"\n");
					break;
			case 3: sword_name=YEL"出跸刀"NOR; 
		...
		//		message("vision", HIW"\n只见一道雪亮的白光划过" +victim_name+ HIW"的胸膛\n"NOR, environment());
				...
				case 2:	victim->start_call_out( (: call_other, __FILE__, "remove_effect", victim :), 50);
						...
						victim->add_temp("apply/armor_vs_force", -random(800));
						tell_object(victim, CYN"你身上的铠甲不胜斩铁剑的重创，防护能力消失甚多！\n");
						break;
				//出跸刀：神秘
				case 3:	tell_object(victim, YEL"你只觉出跸刀滑过你的皮肤，居然丝毫不觉疼痛，甚至还有种很享受、很舒服的感觉。\n"); 
						message_vision(CYN""+victim_name+"脸露微笑，仿佛很受用的样子...\n"NOR,environment());
						if (random(100)>10)
						{
							if (random(100)>80)
                                victim->apply_condition("tm_poison", random(20)+victim->query_condition("tm_poison"));    
							if (random(100)>80)
								victim->apply_condition("rose_poison", random(20)+victim->query_condition("rose_poison"));    
 							if (random(100)>80)
                               victim->apply_condition("ninemoonpoison", random(20)+victim->query_condition("ninemoonpoison"));    
							if (random(100)>80)
                                victim->apply_condition("blockweb", random(20)+victim->query_condition("blockweb"));    
							if (random(100)>80)
                                victim->apply_condition("snake_poison", random(20)+victim->query_condition("snake_poison"));    
							if (random(100)>80)
                                victim->apply_condition("zhang_qi", random(20)+victim->query_condition("zhang_qi"));    
							if (random(100)>80)
                                victim->apply_condition("xiezi_poison", random(20)+victim->query_condition("xiezi_poison"));    
							if (random(100)>80)
                                victim->apply_condition("super_poisons", random(20)+victim->query_condition("super_poisons"));    
							if (random(100)>80)
                                victim->apply_condition("xx_poison", random(20)+victim->query_condition("xx_poison"));    
							if (random(100)>80)
                                victim->start_busy(5+random(10));    
							if (random(100)>80)
							{
                                victim->apply_condition("doom", 2+random(3) );    
								victim->set_temp("doomer", player);
								tell_object(victim, HIB "你感到眉头之处一紧一松，一股强大的压力压得你喘不过气来。\n" NOR );
								message_vision("\n"+name()+BLK"手中"NOR+YEL"出跸刀"NOR+BLK"一垂，喃喃念了几句咒语，只见"+victim->name()+"脚下的地上浮起一道黑气，笼罩在"+victim->name()+"头顶。\n"NOR,environment(victim));
								tell_object(victim, HIB "你耳边仿佛有个又阴又冷，仿佛来自地狱的声音对你说：“你只有" + chinese_number( victim->query_condition("doom") ) + "秒钟的时间了。”\n" NOR );
								if (!userp(victim))
				...
				case 4:	suck = victim->query("eff_kee")/10;
						...
						break;
				//温柔
				case 5:	if (random(100)>20){
						message_vision(BLU"只见刀光轻轻一闪，"+victim->name()+BLU"已经倒下，身上却没有半点伤痕。\n"NOR, environment());
						victim->unconcious();
						};
						break;
				}
