...
void create()
{
	set("short", RED"火焰洞"NOR);
	set("long", @LONG
到处都是大火，烤得你非常难受，几乎窒息。一头九头火龙趁机扑了过来。
LONG
	);
	set("exits",([
...
int valid_leave(object who,string dir)
{
	...
	if( objectp(dragon = present("fire dragon",environment(who) ) ) 
&& dir=="out" )
...
int sinking(object ob)
{
	int mykee,mycps,damage;
		...
		return 0;
	mycps=ob->query("cps");
	message_vision(HIW"$N感觉越来越难受, 身体热得难受。\n", ob);
	message_vision(RED"$N不时被涌来的火焰呛进口中！\n"NOR, ob);
...
int no_quit()
{
        ...
        {
		message_vision("$N想溜之大吉，但困在火浪中，哪里作得了主。\n", ob);
                return 0;
        }
        return 1;
}
