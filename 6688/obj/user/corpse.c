...
void decay(int phase)
{
        ...
        switch(phase) {
                case 1:
                        ...
                        set("long",     "这具尸体显然已经躺在这里有一段时间了，正散发著一股腐尸的味
...
道。\n");
                ...
                case 3:
                        ...
                        if( environment() ) {
                                ...
                                if(environment()->is_character())       //防止get corpse来获取宝贝

//的方法
                                  {
                                for(i=0; i<sizeof(inv); i++) if(inv[i]) destruct(inv[i]);
                                  }
                                else
                                for(i=0; i<sizeof(inv); i++) if(inv[i]) 
