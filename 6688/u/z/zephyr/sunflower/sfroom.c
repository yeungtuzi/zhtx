...
int full(mixed ob)
{
        ...
        ob->clear_condition("sheng_si");//生死符
...
int creature_die(object me)
{
	...
	if (random(100)>20)
	{
        ...
        message("system", HIR "\n你被打晕了....\n\n" NOR,me);
        //tell_room(this_object(),me->query("name")+"被家丁扒了个精光，架了出去，扔出了山庄。\n");
        full(me);
        me->unconcious();
...
/*        object ob;
        ...
        if( (wiz_level(ob) || raw_wiz_level(ob)) && wiz_level(ob)<3 && raw_wiz_level(ob)<
5 )
        {
                        ob->move("/d/snow/inn");
                        return;
