...
寺内学艺。");
        ...
        if ( ob_fam["generation"] == 0 )
        {
                ...
                command("chat 哈哈哈哈");
                command("chat 我佛释伽牟尼又得一传人，当真可喜可贺");
                return;
        }
...
先学做人。");
        ...
        if (ob->query_int() < 45) {
                command("say 我佛释伽牟尼大智大慧，方能在菩提树下悟道升天。");
                command("say " + RANK_D->query_respect(ob) + "的悟性还是不够啊,
请回吧。");
                return;
        }
...
吧，哈哈哈 !");
                        command("say 我佛又得传人，真是可喜可贺 !");

                        new_name = "渡" + name[2..3];
                        ob->set("name", new_name+"长老");
...
void recruit_apprentice(object ob)
{
	...
	{
        	ob->set("title",HIR "少林" NOR + CYN "长老" NOR);
		return;
	}
...
void re_rank(object student)
{
	...
	{
        	student->set("title",HIR "少林" NOR + CYN "长老" NOR);
		return;
	}
