...
valid_combine(string combo) {
	string skill_file = SKILL_D(combo);
	if(skill_file) {
		return (skill_file)->valid_enable("cuff")   || 
		       (skill_file)->valid_enable("strike") ||
		       (skill_file)->valid_enable("finger") ||
		       (skill_file)->valid_enable("claw")   ||
		       (skill_file)->valid_enable("hand");	
	}
	else return 0;
