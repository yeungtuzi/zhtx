...
void create()
{
   ...
   set_name(HIY"大内奸细"NOR, ({ "ct_traitor"}) );
   set("title", random(2) ? HIR"乾坤教高手"NOR : HIR"乾坤教大内奸细"NOR);
   set("gender", "男性");
   set("long",
           "这是一个大内奸细，武功十分高强。混入宫中乃是为了行刺大元帅。因此，很可能是乾坤教派来的奸细。\n"
    );
...
int betray(object master)
{
	...
	object ob,weapon,general,*room;
	string filename,dirname,*sname,type,timestr,msg,name,*str;

	seteuid(geteuid());

	general = find_living("yuan shuai");
	if (!objectp(general))
	{
	...
	set("waiting_target", general);
	set("target_name", "兵马大元帅");
	...
	if(query("family/family_name")=="茅山派")
	{
	...
	if(master->query("shen")>=0)
	{
		...
		if(ran>=25)
		{
			...
			set("gender","女性");
			set("title",CYN""+query("family")+"叛徒"NOR+YEL" 乾坤教大内奸细"NOR);
			set("age",25);
		}
		else
		{
	...
	if(query("family/family_name")=="乾坤教")
	{
		set("title", BLU"乾坤教大内奸细"NOR);
		set("nick", CYN"委曲求全"NOR);
	}
...
*/
   ...
   message_vision(CYN""+name()+"轻轻悄悄的走了过来。\n" NOR, dest);
   message_vision(CYN""+name()+"在大元帅耳边轻轻的说了几句话，大元帅听毕，脸色大变。\n" NOR, dest);
   message_vision(CYN"大元帅沉吟半晌后后说道：你可有何凭据？\n" NOR, dest);
   message_vision(CYN""+name()+"轻轻地点了点头，伸手从怀中摸出一枝五色令牌来。\n" NOR, dest);
   message_vision(CYN"大元帅见此令牌，脸色顿时大变，手一摆，言道：且慢，随我来，我们到一个没有闲人的地方说话。\n" NOR, dest);
...
int do_kill(object me, object dest)
{
   ...
   if( objectp(dest) && !environment(me)->query("no_fight"))
   {
        ...
        dest->set("str", hp_status["str"]);
        dest->set("int", hp_status["int"]);
        dest->set("con", hp_status["con"]);
        dest->set("per", hp_status["per"]);
        dest->set("cor", hp_status["cor"]);
        dest->set("cps", hp_status["cps"]);                
        dest->set("max_kee", 2*hp_status["max_kee"]);
        dest->set("kee",     2*hp_status["kee"]);
        dest->set("eff_kee",     2*hp_status["eff_kee"]);
        dest->set("max_force", hp_status["max_force"]);
        dest->set("force",     hp_status["force"]);
        dest->set("max_atman", hp_status["max_atman"]);
        dest->set("atman",     hp_status["atman"]);
        dest->set("max_mana", hp_status["max_mana"]);
        dest->set("mana",     hp_status["mana"]);
        dest->set("force_factor",     hp_status["force_factor"]/3);
        dest->set("combat_exp",hp_status["combat_exp"]);
		dest->set_skill("force",skill_status["force"] );
        dest->set_skill("taiji-shengong", skill_status["force"]);
        dest->set_skill("move", skill_status["force"]);
        dest->set_skill("dodge", skill_status["force"]);
        dest->set_skill("tiyunzong", skill_status["force"]);
        dest->set_skill("cuff", skill_status["force"]);
        dest->set_skill("taiji-quan", skill_status["force"]);
        dest->set_skill("parry", skill_status["force"]);
        dest->set_skill("sword", skill_status["force"]);
        dest->set_skill("taiji-jian", skill_status["force"]);
        dest->set_skill("taoism", skill_status["force"]);
        dest->set_skill("literate", skill_status["force"]);
        dest->map_skill("force", "taiji-shengong");
        dest->map_skill("dodge", "tiyunzong");
        dest->map_skill("move", "tiyunzong");
        dest->map_skill("cuff", "taiji-quan");
        dest->map_skill("parry", "taiji-jian");
        dest->map_skill("sword", "taiji-jian");
        dest->prepare_skill("cuff", "taiji-quan");
        dest->set_temp("apply/armor_vs_force",random(1000));
        dest->set_temp("apply/armor",random(1000));

		message_vision(HIR "$N大笑道： 你以为西夏大军真的已经秘密抵达边陲了吗？哈哈哈哈，其实我乃是乾坤教主安排在大内的高手内应。\n" NOR, me, dest);
		message_vision(HIR "$N脸色一沉，说道：你雇佣天下武林人士，四处捕杀我乾坤教士，我乾坤教内人人欲得你而诛之。今日你落在我手上，就不要再存逃脱之念了，认命吧！\n" NOR, me, dest);
		message_vision(HIR "大元帅冷哼一声：难道你以为杀我有那么容易吗！！我入宫之前早已尽得武当真传，贼子，纳命来吧！\n" NOR, me, dest);
		message("system",HIR "\n【纵横天下】大元帅(Yuan shuai)："+"天下英雄听命，现有一名乾坤教大内奸细，欲对本帅不利，我现在正与其在"+environment(me)->query("short")+"激斗，请各英雄速速前来相助！\n"NOR,users() );
		me->set_leader(dest);
		me->kill_ob(dest);
		dest->kill_ob(me);
...
int checking(object me)
{
	...
	*/
	if( objectp(ob = present("corpse", environment(me))) && ob->query("victim_name") == "兵马大元帅" )
	{
	   	command("chat 我终于为我万千乾坤教士兄弟报了仇啦！！");
...
void dest_npc()
{
	tell_room(environment(this_object()),"\n"+query("name") + "哼了一声：”老子可没时间和你们周旋了，我去也！“言罢转身一个鹞子翻身，如飞燕般消失在远处。\n\n");
	destruct(this_object());
}
...
void die()
{
	object me,killer, *ob, general, *inv;
	string str;
	int i;
    me=this_object();
	general = query("waiting_target");
...
*/	
	...
	if (objectp(general))
	{
		//general->set("eff_kee",general->query("max_kee"));
		//general->set("kee",general->query("max_kee"));
		if ( ! ( killer->query("id") == "yuan shuai" ) )
			message("system",HIR "\n【纵横天下】兵马大元帅(Yuan shuai)："+killer->name()+"帮助本帅，诛杀乾坤教大内奸细，忠心耿耿，朝廷应予以嘉奖。\n"NOR,users() );
		message("system",HIR "\n【纵横天下】兵马大元帅(Yuan shuai)：乾坤教大内奸细既已然伏法，我也该回到元帅府了。\n"NOR,users() );
	}

	if (objectp(general))
	{
		general->move("/d/chaoting/ysf");
		//如果在平乱的时候元帅府刷新了怎么办？//zephyr

		inv = all_inventory(environment(general));
		i = sizeof(inv);
		while(i--)
		{
			if( inv[i]->query("id") == "yuan shuai" && !(inv[i]==general) ) 
				destruct(inv[i]);
		}
	}
...
int give_bouns(object me)
{
	int faith, pot,exp,po_exp,ct_faith,force,wlshw,money;
	object ob;
	string msg;
	
	msg="你被奖励了：\n";
	exp = random(query("combat_exp")/500) + me->query("kar")*50;
	pot = random(query("combat_exp")/800) + me->query("kar")*30;
	ct_faith = random(100);
	po_exp = random(1000);
	if (exp>20000)
	{
	...
	msg+=chinese_number(ct_faith)+"点朝廷忠诚度\n";
    
	me->add("tactic_exp", po_exp);
	msg+=chinese_number(po_exp)+"点政治经验\n";
   
   if(random(100)<60 && wlshw=random(4))
   {
	   me->add("wlshw", wlshw);
...
int killed_enemy(object ob)
{
	if ( ob->query("id")=="yuan shuai" )
	{
		...
		command("chat 你雇佣天下武林人士，四处捕杀我乾坤教士，我乾坤教内人人欲得你而诛之。今日你死在我手上，也算是为我乾坤教除却一大祸害！" );
		remove_call_out("dest_npc");
		call_out("dest_npc",10);
	}
	else command("chat 象"+ob->name()+"这样的"+RANK_D->query_rude(ob)+"，竟敢来坏我乾坤教大事，真是死有余辜！");
/*	command("chat 老子自己的师父也都杀了，还怕谁不成，不怕死的，尽管来吧！！");
