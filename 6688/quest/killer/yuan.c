...
void continue_reward(object me)
{
        ...
        if ( t > 19 && random(me->query_kar()) > 25 )
//原先给丸子概率太高，泛滥了
        {
     ...
     if(t>19 && random(me->query("kar"))>25)
     {      wlsw = 1+random(2);
         me->add("wlshw",wlsw);
      str += ", " + wlsw + "点武林声望";
       me->set_temp("cttask/gift2",1);
     }
            if(t>100 && random(me->query("kar"))>27 )
            { 
              luck = random(4);
              if( luck == 0) obj = new("u/m/masterall/obj/lfwan.c"); 
              if( luck == 1) obj = new("u/m/masterall/obj/jndan.c"); 
              if( luck == 2) obj = new("u/m/masterall/obj/xyring.c"); 
              if( luck == 3) obj = new("u/m/masterall/obj/mbox.c"); 
              obj->move(me);
              obj->set("owner", me);
              if( luck == 0) str += "，" + obj->name() + "一枚";
              if( luck == 1) str += "，" + obj->name() + "一枚";
              if( luck == 2) str += "，" + obj->name() + "一枚";
              if( luck == 3) str += "，" + obj->name() + "一个";
              me->set_temp("cttask/gift2",1);
            }
