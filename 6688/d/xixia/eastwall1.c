...
LONG
        ...
        set("exits", ([ /* sizeof() == 5 */
	  "north" : __DIR__"eastwall2",
	  "northdown" : __DIR__"eastgate",
	  "enter" : __DIR__"easttower",
	  "south" : __DIR__"eastwall3",
...
int action_gate(string arg)
{
	...
	if (!(gate = find_object(__DIR__"southgate")))
		if (!(gate = new(__DIR__"southgate")))
	...
	if (arg == "open") {
		if (gate->query("exits/south") && bridge->query("exits/north"))
			...
			gate->set("exits/south", __DIR__"southbridge");
			bridge->set("exits/north", __DIR__"southgate");
			return 1;
		}
	}
	else {
...
int action_bridge(string arg)
{
	...
	if (!(bridge = find_object(__DIR__"eastbridge")))
		if (!(bridge = new(__DIR__"eastbridge")))
			return notify_fail("找不到吊桥啦！\n");
	if (!(road = find_object(__DIR__"eastroad1")))
		if (!(road = new(__DIR__"easthroad1")))
	...
	if (arg == "down") {
		if (bridge->query("exits/south") && road->query("exits/north"))
			...
			bridge->set("exits/south", __DIR__"eastroad1");
