...
void relay_emote(object ob, string verb)
{
   ...
   switch(random(50))
   {
    ...
    case 8:
    command("cry");
   command ("chat 无记快来啊，"+ ob->query("name") + "想要羞辱我！\n") ;
...
varargs void drool(string msg, string who)
{
	...
	if( (strsrch(msg, "kiss") >= 0)|| (strsrch(msg, "love") >= 0)|| (strsrch(msg, "吻") >= 0)
		...
		case 7:command ("chat 无记快来啊，"+ who + "想要羞辱我！\n") ;
		...
		case 15:command("chat 嗯，"+who+"是天底下最好的人了！\n"); break;
	                case 16:command ("chat 无记快来啊，"+ who + "欺负我！\n") ; break;
		}
		return;
	}
	...
	{	
		...
		case 13:command ("chat 无记快来啊，"+ who + "又想要羞辱我！\n") ; break;
		}
	}
	...
	switch(random(80)) {
        ...
        case 26:	command ("chat 无记快来啊，"+ who + "又来欺侮我！\n") ; break;
...
{
	command( "sigh");
	return("咦，你也知道她？她是我妈咪啦,可惜她很久没有上网了。。。\n ");
}
string ask_father()
{	object dest;
	int i,havepock;
	 ...
	 object me;
         object room;
         me = this_player();
	 room=load_object("class/qingcheng/workroom");
	 inv = all_inventory(room);
	 for(i=0; i<sizeof(inv); i++)
	   if((string)(inv[i]->query("id")) == "pock") havepock=1;
 	dest = find_player("pock");
	if(dest)
	{
	 if(me->query("id")=="pock")
		return("老爸你今天怎么啦?你是不是发烧了?你不是在这里吗?");
	 else if(havepock)
		return("他不是在这里吗?你还问我干吗?\n");
	else
	dest->move("/class/qingcheng/workroom");
	write("夜莺说:你在这里稍等一会儿,我去把他叫回来!\n");
	write("夜莺蹦蹦跳跳地走了出去.\n");
	write("只一会儿,夜莺和春梦了无痕走了进来.\n");
	write("春梦了无痕说道:这位" + RANK_D->query_respect(this_player())+"，找我什么事?\n");
	return("我回来了!!!\n");
	}
	else return("他很忙的,恐怕没有时间见你!\n");
}
...
{	object who;
	...
	{
 	        if(me->query_temp("nomemnod"))
		{
		tell_object(me, "\n夜莺嫣然一笑，说：好吧，既然无记叫你来，那我就听你的。\n");
		tell_object(me, "请键入：杀 目标中文名字\n");
		me->delete_temp("nomemnod");
		me->set_temp("fee_paid",1);
		return 1;
		}
		else 
		{
		say("\n夜莺摇摇头，笑道：不好吧，杀人的事我爸不会同意的。\n");
		me->set_temp("fee_paid2",1);
		tell_object(me, "\n夜莺嫣然一笑，说：不过，我可以开个小玩笑。\n");
	...
	if ((string)ob->query("name")==HIW "冰玉戒指" NOR ||(string)ob->query("name")==YEL "疾风靴" NOR
		||(string)ob->query("name")==HIW "翡翠玉带" NOR||(string)ob->query("name")==MAG "钻石胸针" NOR) 
	{
	if ((string)ob->query("name")== HIW "冰玉戒指" NOR ) 
	{
	message_vision("\n$N猛的扬起头，用惊喜的眼光看着$n，说：\n"
	+"咦，这不是无记送我的戒指嘛，是他要你带来的么? 真谢谢你啊...\n",this_object(),me);
	}
	if ((string)ob->query("name")==YEL "疾风靴" NOR) 
	{
	message_vision("\n$N猛的扬起头，用惊喜的眼光看着$n，说：\n"
	+"咦，这不是无记送我的疾风鞋嘛，是他要你带来的么? 真谢谢你啊...\n",this_object(),me);
	}
	if ((string)ob->query("name")==HIW "翡翠玉带" NOR) 
	{
	message_vision("\n$N猛的扬起头，用惊奇的眼光看着$n，说：\n"
	+"咦，这不是我的玉带嘛，怎么你...\n",this_object(),me);
	}
	if ((string)ob->query("name")==MAG "钻石胸针" NOR) 
	{
	message_vision("\n$N猛的扬起头，用惊奇的眼光看着$n，说：\n"
	+"咦，这不是我的贴身胸针吗，怎么你...\n",this_object(),me);
	}
	return 1; 
     ...
     if((string)ob->query("name")==MAG"紫罗兰"NOR)
	 {
           ...
           if( this_player()->query_temp("marks/谜") ) {
                 write("夜莺笑了笑，说：你先把上个谜猜出来再说。\n");
                 return 1;
           }
           else {
                 write("夜莺笑了笑，说：多谢这位" + 
                 ...
                 switch(random(20)) {
                   case 0:
                       write("失忆。(打一本泥潭玩家名)\n");
                       this_player()->set_temp("marks/谜", 1);
                       break;
                   case 1:
                       write("哈哈哈,少少少。(打一本泥潭玩家名)\n");
                   ...
                   case 2:
                       ...
                       break;
                   case 3:
                       write("PC Game 里的生活。(打一本泥潭巫师网名)\n");
                       who->set_temp("marks/谜", 4);
                       break;
                   case 4:
                       write("君不见黄河之水天上来。(打一bbs上常用社交动作)\n");
		       ...
		       break;
                  ...
                  message("vision", "夜莺对着"+who->name()+"嘀咕了几句话。\n",
                          environment(who), ({who}) );
                  remove_call_out("destroying");
                  call_out("destroying", 1, this_object(), ob);
                  return 1;
	...
	{
	message_vision("\n$N缓缓地摇了摇头，柔声说道:不是无记送我的,我不要.\n"
	+"说罢垂下头来，咬了咬牙,幽幽嗔道：死无记，臭无记，为什么还不回来呢?\n",this_object());
	return 0;
	}
}
int do_answer(string arg)
{
        ...
        message("vision", me->name() + "把嘴凑到夜莺的耳边嘀嘀咕咕。\n",
                 environment(me), ({me}) );

        switch (arg) {
          case "无记": soln=1; break;
          case "笑三少" : soln=2; break;
          case "江百胜" : soln=3; break;
          case "游戏人生" : soln=4; break;
          case "灌水" : soln=5; break;
          ...
          case "朱红": soln=16; break;
	  case "少林": soln=17; break;
	  case "李莫愁" : soln=18; break;
	  case "逍遥洞逍遥游" : soln=19; break;
          case "买卖提" : soln=20; break;
          default :
              say("夜莺大笑起来，说道：你说什么呀，牛头不对马嘴。\n"
                  + me->name() + "的脸被臊得通红。\n");
	      this_player()->add("marks/shuang",-1);
              return 1;
        }

        if (riddle==soln) {
          this_player()->set_temp("marks/谜", 0);
          say ("夜莺对" + me->name() + "竖起大姆指：居然让你猜着了。应该给你个奖励。\n");
          this_player()->add("combat_exp",random(80));
          write ("你的经验增加了！\n");
	  this_player()->add("marks/shuang",1);
...
int do_name(string target)
{
	...
	if(me->query("waiting_target"))
		return notify_fail("夜莺：我现在正忙着哪，您呆会儿再来吧！\n");
	if( !this_player()->query_temp("fee_paid") ) 
		return notify_fail("夜莺打了个呵欠，左手轻轻掩住樱桃小口，柔声道：\n"+
		"不好吧，帮你杀了人我又有什么好处呢？\n");
	if( !target || target==" ")
		return notify_fail("夜莺打了个呵欠，左手轻轻掩住樱桃小口，柔声道：\n"
		+"请告诉我那人的名字，我可没功夫跟你闲聊。\n");
	...
	if( !dest )
		return notify_fail("夜莺做了个鬼脸：不太好啊，好像他不在喔。\n");

	if (dest->is_ghost())
		return notify_fail("夜莺说：你黑，你太黑，你太太黑，你太太太黑！他已经死啦！\n");
	me->set("waiting_target", dest->query("id"));
	me->set("target_name", target);
	me->delete("chat_chance");
	me->delete("chat_msg");
	message("vision",
	HIY "\n夜莺正要离开，忽又回首道：你别走，就在这儿静侯佳音吧。\n" NOR,
	environment(), me );
	me->set("pursuer", 1);
...
int do_name2(string target)
{
	...
	if(!this_player()->query_temp("fee_paid2") ) 
		return notify_fail("夜莺打了个呵欠，左手轻轻掩住樱桃小口，柔声道：\n"+
		"不好吧，那样做我又有什么好处呢？\n");
        if(!this_player()->query_temp("nomemnod2"))
		return notify_fail("\n夜莺摇摇头，笑道：不好吧，打人的事无记不会同意的。\n");
        if( !target || target==" ")
		return notify_fail("夜莺打了个呵欠，左手轻轻掩住樱桃小口，柔声道：\n"
		+"请告诉我那人的名字，我可没功夫跟你闲聊。\n");
	for(i=0; i<sizeof(all); i++)
	...
	if( !dest )
		return notify_fail("夜莺做了个鬼脸：不太好啊，好像他不在喔。\n");
	if (dest->is_ghost())
		return notify_fail("夜莺说：你好坏哟，他已经死啦！\n");
	me->set("waiting_target2", dest->query("id"));
        this_player()->delete_temp("nomemnod2");
        this_player()->delete_temp("fee_paid2");
	message("vision",
		HIM "\n夜莺以袖掩口，笑道：嘻嘻，好像成了耶，你怎么谢我呀？\n" NOR,
		environment());
...
int do_faint(object me)
{
	...
	message_vision(HIC "$N对$n说道：有人请我来打晕你，对不起啊。。。\n"
        ...
        dest->unconcious();
	this_object()->move("/class/qingcheng/workroom");
 	return 1;
}
...
int do_kill(object me, object dest)
{
	...
	{
		message_vision(HIR "$N对$n说道：无记要我来杀你，对不起啊。。。\n"
			+RANK_D->query_rude(dest)+"，你就认命吧！\n" NOR, me, dest);
			if(wizardp(dest))		
	                { 
			dest->set("immortal",0);
...
int do_back(object me)
{
	...
	me->move("/class/qingcheng/workroom");
	message("vision", "夜莺走了进来，幽幽道：唉，无记真不乖，害得我又杀了人。\n", 
		environment(), me );
	me->set_leader(0);
	set("chat_chance", 3);
	set("chat_msg", ({
