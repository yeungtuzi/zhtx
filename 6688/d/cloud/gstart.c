...
mapping yizhan=([
	"suzhou" : "/d/suzhou/gstart",
	"fuzhou"  : "/d/qingcheng/gstart",
]);
...
void create()
{
	set("short","镖局驿站");
	set("long", @LONG
        这里是本地镖局和别处镖局连通的驿站，镖局中的人可以从这里免费乘车
去往外地镖局。墙上有一块牌子(paizi). 
还有一个镖局中的车夫坐在旁边等着。
LONG
);
...
int do_qu(string arg)
{
	...
	if(!who->query("class") || who->query("class")!="guard" )
		return notify_fail("你不是镖局中人，怎么能乘这儿的车呢？\n");
	if ( !(che=find_object(__DIR__"che2"))) che=clone_object(__DIR__"che2");
	if (che->query("youren"))	
	{
...
int do_look(string arg)
{
	...
	if(arg=="paizi")
	{
		...
		while(i--)
		{
			str=sprintf("%12s   需时间:%4d秒\n",zhan[i],time[zhan[i]]*5);
			tell_object(who,str);
		}
		return 1;
	}
