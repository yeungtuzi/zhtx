...
{
	int i,amun;
	set_name( "刘麻子", ({ "liu", "liu ma zi" }) );
	set("long", "这是赌坊的职业赌手，骰子上的功夫出神入化。\n");
...
void greeting(object ob)
{
        ...
        switch( random(10) ) {
                case 0:
                        say( "刘麻子笑咪咪地说道：这位" + RANK_D->query_respect(ob)
                                + "，过来赌两手，开开心吧。\n");
                        break;
                case 1:
                        say( "刘麻子晃了晃手中的骰子，叫道：这位" + RANK_D->query_respect(ob)
                                + "，小赌可以养家糊口，大赌可以定国安邦。来吧！\n");
                        break;
                case 2:
                        say( "刘麻子说道：这位" + RANK_D->query_respect(ob)
                                + "，进来！进来！ 输了算我的！\n");
                        break;
        }
...
inv = all_inventory(environment(this_object()));
		...
		if((int) inv[i]->query("gamble/type") == tot)
		{
                ...
                inv[i]->set_temp("win-time",(int)inv[i]->query_temp("win-time")+8);
...
// also increae his betting skill
			...
			((int) inv[i]->query("gamble/type") == 1 && tot > 10 ))
                ...
                inv[i]->set_temp("win-time",(int)inv[i]->query_temp("win-time")+1);
...
int accept_object(object who, object ob)
{
	...
	tell_object(who,"刘麻子悄悄告诉你：下手儿压"+message+"，一定赢！\n");
	message_vision("$N会心的微笑起来．．\n",who);
	
                        return 1;
                } else {
                    say("刘麻子说道：这点钱你还是自己用吧！\n");
                        return 0;
                }
        return 0;
}
...
int check(object obj)
{
	...
	wintimes=(int)obj->query_temp("win-time");
	if (wintimes<10) return 0;
	inv=all_inventory(obj);
        for(i=0;i<sizeof(inv);i++)
        {
		if(inv[i]->query("name")=="金元宝") gold=inv[i];
                if(inv[i]->query("id")=="qing tie") obj->set_temp("test-ok",1);
               
         }
         if(!gold||(gold->query_amount()<30)) return 0;
         if(!obj->query_temp("test-ok")) return 0;
         obj->set_temp("can-enter",1);
         tell_object(obj,"刘麻子悄悄对你说:“大管家有请，客官请进。”\n");
         return 1;
}