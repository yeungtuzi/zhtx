...
//魔法盾牌不起作用      
        if( type=="kee" && !query_temp("jiuyang-shield") && query_temp("manashield") && !this_object()->query_skill("iron-cloth") && this_object()->query_skill_mapped("spells")=="necromancy" )
        {
                mana = query("mana");
                if( damage )
        ...
        if ( me->query("cttask/get") )
                {
                        ...
                        if ( killer == me->query_temp("last_damage_from") || killer == me->query_temp("last_opponent") )
                             {
                       ...
                       me->add(((int)me->query("learned_points")-(int)me->query("potential"))/2);
