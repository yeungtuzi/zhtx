...
void create()
{
    ...
    //          "机关"   : (: ask3 :),
              "左右互搏": (: ask2 :),
                "出墓": "祖师遗命，凡我古墓弟子都不得擅自出去行走江湖，除非，有一个男子肯为你舍弃自
...
void attempt_apprentice(object ob)
{
        ...
        if ( (int)ob->query_dex()<25 && (int)ob->query_skill("dodge",1)<100 )
                {
                command("say" + "我古墓派的功夫以轻灵为主，" + RANK_D->query_respect(ob) + "似乎不
...
{mapping fam;
        ...
        if (me->query_skill("yunu-jianfa",1)<100 
         || me->query_skill("quanzhen-jian",1)<100)               
        { message_vision("$N练了半天，还是没练会。”\n",me);
               
        return "也许是你玉女剑法和全真剑法不够，静不下心来练吧。\n";}
        if (me->query("combat_exp")>15000000)
        {message_vision("$N练了半天，还是没练会。”\n",me);
        return "也许是你经验太高了，不习惯于左右互搏吧。\n";}
...
int quest_punish(object who) {
        message_vision( (this_object())->query("name")+"向$N一甩袍袖，说道： 真笨，你好好学学功夫再
