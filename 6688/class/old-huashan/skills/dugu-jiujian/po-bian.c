...
int perform(object me, object target)
{
        ...
        seteuid(ROOT_UID);
        if( !me->is_fighting() )
        	return notify_fail("「破鞭式」只能在战斗中施用。\n");        	                     
	if( me->is_busy())
	        return notify_fail("你正忙着呢，来不及用独孤九剑。\n");
	if (!objectp(ob = me->query_temp("weapon"))
        || (string)ob->query("skill_type") != "sword")
                return notify_fail("你手中无剑，如何使用独孤九剑的「破鞭式」。\n");	
        ...
        if( skill < 150 ) return notify_fail("你的「独孤九剑」招式不够娴熟，使不出「破鞭式」。\n");        	                     
        if( me->query_skill("dagger",1)<150) return notify_fail("你的基本功不够扎实，无法使用「破鞭式」。\n");
           
        if( me->query("betrayer"))
            return notify_fail("你回想起以前在师门中所曾经受过的好处，再也无法使出独孤九剑来了。\n");
            
        if(
        //! check_weapon(target,"club") 
      ...
      //  && ! check_weapon(target,"spear")
        ...
        && ! check_weapon(target,"hook")
        && ! check_weapon(target,"axe")) 
            return notify_fail("对手手中并没有短兵刃，你如何使用「破鞭式」？\n");
        message_vision(HIY"$N使出独孤九剑「破鞭式」，静气凝神，仔细观察$n的一举一动，试图找出$n的破绽.\n" NOR, me,target);
...
int do_prepare(object me,object target)
{
	...
	if( left_busy > 3 )
	{
		...
		if(//! check_weapon(target,"club") 
        	...
        	&& ! check_weapon(target,"axe")) 
        	{
	                tell_object(me,"对手手中并没有短兵刃，你如何使用「破鞭式」？\n");
