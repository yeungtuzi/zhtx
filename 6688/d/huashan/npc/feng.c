...
void attempt_apprentice(object ob)
{
 	...
 	if(this_player()->query("mark/secret/6_ask_fox")
        ...
        if ((string)ob->query("family/family_name")!="华山派剑宗") 
        {
	...
	{
		command("say 你的剑术修为不够。");
	}

	if( ob->query_skill("sword") < 300 )
	{
		command("say 你的剑术修为不够。");
	}
        ...
        if ((int)this_player()->query_spi() < 45 )
        {
                command("say 学习独孤剑主要要有好的悟性，"+RANK_D->query_respect(ob)+"资质不够。");
                return;
        }
        if((int)ob->query("shen") < 3000000) 
        {
