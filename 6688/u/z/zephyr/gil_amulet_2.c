...
void create()
{
        set_name(YEL"纵横"BLU"兵器谱"NOR, ({ "bingqi pu" , "zongheng bingqipu", "bingqipu", "amulet", "pu" }));
     set_weight(50);
     set("unpawnable",1);
...
破残：原为名刀出跸，乃一柄黄杨木雕的弯刀，无鞘，刀身长三尺半，上面雕
        ...
        刀，但持魔刀者却丧命于这柄出跸刀之下。
        此刀破后，因机缘巧合回归吾手，于天山北麓寒于洞，费三年心血，合万年
        冰魄，才有此刀。此刀经此大难，已失往日犀利，攻击能力忽强忽弱，名之：
        破残
        无视对方防御力，冰冻属性攻击
    ...
    int i, all, player_gil_flag, heal_kee, maxkee, effkee, kee;
        ...
        {       
                if( me->is_fighting() && (gil) && !me->query_condition("zhaohuan_weaponor" ) )
                {
                        //只在战斗之初判断一次是否召唤基加美修
                        gil = 0;
                        if( random(100)>70 && random(player_gil_flag) > 700)
                        {       
                                ...
                                all = sizeof(enemy);
                                if( !all ) return;
                                obod=new(__DIR__"gilgamesh_2");
                                obod->move(environment(me));
                                
								me->apply_condition("zhaohuan_weaponor", 10+random(10) );
                ...
                if( !living(me) && (wake) )
                {
                        ...
                        if( random(100)>30 && random(player_gil_flag) > 1500)
                        {
...
int do_feel(string arg)
{
	...
	int value = 25 * (player_gil_flag - 1000) /  player_gil_flag;
	int w = 70 * (player_gil_flag - 1500) /  player_gil_flag;
