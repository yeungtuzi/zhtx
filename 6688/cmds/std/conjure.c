...
//added by yeung to prevent conjureing while using essencemagic
	if( me->query_temp("conjureing") )
		return notify_fail("你现在无法使用神通。\n");
        ...
        if( me->query_condition("blockweb") )
                return notify_fail("你的灵力受到一种莫名的干扰,无法运用.\n");

        if( me->query_condition("vspo") ) 
                return notify_fail("你被对手的独孤九剑所震慑，竟然一时难以使用神通。\n");
