...
北边你看到了一个高耸的城楼被一条白带环绕着，原来这
里已经离西夏国的都城──中兴府不远了。
LONG);
	set("outdoors", "xixia");
	set("exits", ([ /* sizeof() == 1 */
  		"north" : __DIR__"southbridge",
...
int do_jump(string arg)
{
	string skill, desc, weapon_type;
        ...
        if( !objectp(wall = find_object(__DIR__"southwall1")))
                if( !objectp(wall = new(__DIR__"southwall1")))
                        return 0;
