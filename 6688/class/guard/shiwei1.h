//redshiwei.h for new redshiwei to fight and for redshiwei to greet players 
#include <ansi.h>
...
int accept_object(object ob, object obj)
{
	...
	if ( ob->query_temp("have_letter") && present("letter1", ob) ) 
	{
		command("say 怎麽样，你拿我的推荐信去拜师了吗 ?");
		return 0;
	}

	if((obj->query("id") == "yu pei")
	&& ob_fam["generation"] == my_fam["generation"] + 1
	&& !ob->query_temp("have_letter") ) 
	{
		ob->set_temp("fight_ok",1);
	        command("say 好，既然已得到王爷许可，我们就来验证一下武功。");
		remove_call_out("destroying");
		call_out("destroying", 1, me, obj);
		return 1;
	}
        
	if (obj->query("id") == "letter0" 
	&& ob->query_temp("have_letter") )
	{
...
int checking(object me, object ob)
{
	...
	{
		command("say 青出於蓝胜於蓝，不愧是宁王府的后起之秀! 恭喜你了 !\n");
...
void attempt_apprentice(object ob)
{
	...
	if (!(ob_fam = ob->query("family")) || ob_fam["family_name"] != "宁王府")
	{
...
void recruit_apprentice(object ob)
{
        ...
        ob->set("title",HIR "宁王府" NOR + CYN "实习卫士"NOR);
        return;
...
{
        student->set("title",HIR "宁王府" NOR + CYN "实习卫士" NOR);
	return;
}
...
void greeting()
{
	...
	ob=this_player();
        ...
        switch( random(3) ) {
                case 0:
                        say( "红衣侍卫笑道：这位" + RANK_D->query_respect(ob)
                                + "，辛苦了。\n");
                        break;
                case 1:
                        say( "红衣侍卫说道：这位" + RANK_D->query_respect(ob)
                                + "，你可知道王府中藏着许多的秘密？\n");
                        break; 
                case 2:
                        say( "红衣侍卫说道：这位" + RANK_D->query_respect(ob)
                                + "，经验不能只靠完成任务，要多探谜才能涨得快。\n");
			 break;
        }
}
