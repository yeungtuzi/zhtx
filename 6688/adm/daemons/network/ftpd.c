...
// restrice retr cmd to be only used in /u/ 
int zh_check_valid_read( string fname, int fd) {
        int res;
	...
	if (strncmp(fname, HOME_DIR( UNAME ), strlen(HOME_DIR( UNAME ))) == 0)
		return res;
	else if( strncmp(fname,"/open",strlen("/open")) == 0) 
		return res;
...
string ls( string path, int column, int fd ) {
	...
	for (i = 0; i < s; i++) {
		...
		if (j == -2) {
			/* directory */
			files[i] = sprintf("drwxrwsr-x %9s %9s	   <DIR>  %12s  %s/",
				  creator, domain, tmp, (xfiles[i])[0]);
		} else {
			/* file */
			stats = stat(path + (xfiles[i])[0]);
			files[i] = sprintf("-rw%crw-r-- %9s %9s  %8d  %12s  %s",
...
*/
			...
			} else if ( !zh_check_valid_read( tmp, fd) )  {
				PERMISSION_DENIED550(command[ 1 ]);
			} else {
#ifdef LOG_TIME
				log_file( LOG_FILE, FTP_TIME );
