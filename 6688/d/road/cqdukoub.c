...
void create()
{
    set("short", "长江北岸");
    set("long", @LONG
你面前就是波涛翻滚的长江(river)。这里的江面约有一里宽。
两岸渡船来来往往，江边一长溜摆满了鱼摊，渔家就将船泊在岸
边。几个破萝支一块木板，板上摆满了活蹦乱跳的白鲢。过去不
远，是河口，不少过江客都在这里等待渡船摆渡。
LONG
    );

    set("exits", ([
                   "northwest" : "/d/road/rcdtodk1",
                   "northeast" : "/d/taoguan/road1",
    ]));
...
void check_trigger()
{
    ...
    if(!query("exits/enter") ) {
        if( !(room = find_object(__DIR__"cqduchuan")) )
            room = load_object(__DIR__"cqduchuan");
        if( room = find_object(__DIR__"cqduchuan") ) {
            if((int)room->query("yell_trigger")==0 ) {
                room->set("yell_trigger", 1);
                set("exits/enter", __DIR__"cqduchuan");
...
void check_trigger_fast()
{
    ...
    if(!query("exits/cqduchuan_fast") ) {
        if( !(room = find_object(__DIR__"cqduchuan_fast")) )
            room = load_object(__DIR__"cqduchuan_fast");
        if( room = find_object(__DIR__"cqduchuan_fast") ) {
            if((int)room->query("yell_trigger_fast")==0 ) {
                room->set("yell_trigger_fast", 1);
                set("exits/fastboat", __DIR__"cqduchuan_fast");
                room->set("exits/out", __FILE__);
                message("vision", "一叶扁舟缓缓地驶了过来，艄公将一块踏脚"
                    "板搭上堤岸，以便乘客\n上下。\n", this_object() );
                message("vision", "艄公将一块踏脚板搭上堤岸，形成一个向上"
...
void arrive()
{
    ...
    if( room = find_object(__DIR__"cqduchuan") )
    {
        room->set("exits/out", __DIR__"cqdukoun");
...
void arrive_fast()
{
    ...
    if( room = find_object(__DIR__"cqduchuan_fast") )
    {
        room->set("exits/out", __DIR__"cqdukoun.c");
        message("vision", "艄公说“到啦，上岸吧”，随即把一块踏脚板"
            "搭上堤岸。\n",room );
    }
