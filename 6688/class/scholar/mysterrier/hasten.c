// hasten.c

#include <ansi.h>

inherit SSERVER;

int perform(object me, object target)
{
        string msg;
        int count,mybusy,skill;
        skill= (int)me->query_skill("mysterrier",1);
        if(skill<250){mybusy=2+random(2);}
        if(skill>249 && skill<281){mybusy=1+random(2);}
        if(skill>280){mybusy=1;} 

        if( !me->is_fighting() )
                return notify_fail("「玄羽乱舞」只能在战斗中使用。\n");

        if( me->query("family/family_name") != "步玄派")
                return notify_fail("你不是步玄派正宗弟子，还没学会使用步玄七诀这一式。\n");

        if( (int)me->query("kee") < 100 ) return notify_fail("你的气不够！\n");
        if( (int)me->query("force") - (int)me->query("max_force") < 70 )
                return notify_fail("你的真气不够！\n");

        msg = HIY "$N使出步玄七诀第一式「玄羽乱舞」，身法陡然加快！\n" NOR;

        message_vision(msg, me);
        count = (int)me->query_skill("mysterrier") / 20 + 2;

        if( count > 12 ) count = 12;
        while( count-- ) {
                object ob;

                me->clean_up_enemy();
                ob = me->select_opponent();
                if( !ob || (environment(ob) != environment(me))) {
                        message_vision(CYN "$N的身形转了几转，倏地停住了脚步。\n" NOR, me);
                        break;
                } else
                        message_vision(CYN "$N迅捷无伦地在$n身旁绕了一圈 ...\n" NOR, me, ob);

                if( !COMBAT_D->fight(me, ob) )
                        message_vision(CYN "但是$N找不到机会出手！\n" NOR, me);
                me->receive_damage("kee", 3);
                me->add("force", -10);
        }

        me->start_busy(mybusy);
            me->set_temp("pfm/scholar",1);
        return 1;
}

