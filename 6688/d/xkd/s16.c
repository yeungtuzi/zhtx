...
void create()
{
      ...
      set("long",HIM"这是一间石室，东面是块打磨光滑的“大石壁”，石壁旁点燃着
...
int do_yandu(string arg)
{
       ...
       if ( me->query_skill("hook",1) < 200 )
                return notify_fail("你的基本功不够扎实，看了也是白看，甭在那里装蒜逞能了。\n");

       if ( me->query_skill("hook",1) > 250 )
       ...
       if ( me->query_skill("literate") ){
        ...
        me->unconcious();
              ...
              printf("你全神贯注的研读石壁上的蝌蚪文，似乎对基本钩法颇有心得。\n");
              me->receive_damage("sen", 100 );
              me->receive_damage("gin", 100 );
              me->add("force", -5 );
              me->add("learned_points", 1 + random(5));
              me->improve_skill("hook", wx + random(50) );
             }
}
