...
HIC"“苍天不负我命，终于让我等到了这一天，好孩子，这对紫金锤今后就交给你了！”\n",NOR,
HIR"“一定要好好珍惜它，记住，‘锤在人在，锤亡人亡！’”\n",NOR,
HIG"“听说我的师兄龙象后来归隐去了侠客岛，如果有机会的话将来你去找他，”\n",NOR,
...
void create()
{
        ...
        create_family("大招寺", 2, "无名前辈");
        set("title", HIB"乾坤教囚徒"NOR);
        set("nickname",HIR "逝去的辉煌，永世的神伤"NOR);
        setup();

        carry_object("/d/qiankun/obj/weapon/zijin-chui")->wield();
}
...
void die()
{
       ...
       command("sigh");
       if(me->query_skill("wuxinglun",1) > 199) 
       {
       me->remove_all_enemy();
       ob->remove_all_enemy();
        revive();
...
{        
        object weapon = new("/d/qiankun/obj/weapon/zijin-chui");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("zijin chui",who) )) 
                {
                weapon->move(who);
                }
