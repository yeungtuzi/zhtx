...
// 允许多人挖宝，各自记录个人的连续完成数目。
// 如果规定时间宝物move了，或者宝物没有被挖到，全部人都算任务序列断。
// yeung 2000-5-12

// 修正宝物连续性判断的BUG
// yeung 2000-5-18

//判断中滤掉 newb 和 sky 区域  -- flyaway
...
LONG
        ...
        if( !arrayp(bad_location) || !sizeof(bad_location) || sizeof(bad_location)>50 )
        {
                bad_location = ({
                        "/d/jingcheng/tamgch",
                });
...
void init()
{
        ...
        add_action("learn_skill","select");

        if( me->query("yin_update_version") < 1 )
        {
                me->delete("wabao_combat_exp");
                me->delete("armor_vs_force");
                me->delete("armor_vs_spells");
                me->set("yin_update_version",1);
        }

        if( !me->query_temp("yin_update") && me->query("combat_exp")>3500000 )
        {
                if( me->query("wabao_combat_exp") > me->query("combat_exp") )
//                                        me->set("combat_exp",me->query("wabao_combat_exp"));
//                me->add_temp("apply/armor_vs_force",me->query("armor_vs_force"));
                me->add_temp("apply/armor_vs_spells",me->query("armor_vs_spells"));
                me->set_temp("yin_update",1);
                command("sigh");
        }
...
int learn_skill(string skill)
{
        object me = this_player();

        if( !me->query_temp("learn_from_yin") ) return 0;
        me->delete_temp("learn_from_yin");

        if( me->query_skill(skill,1) < 150 )
        {
                write("你对这门功夫的理解还不够深！\n");
                return 1;
        }
        message_vision("$n向$N仔细讲解「"+to_chinese(skill)+"」运用技巧，$N听着听着，若有所悟...\n",me,this_object());
        me->improve_skill(skill,1000*me->query("task/task_count"));
        log_file("WABAO_BONUS",sprintf("%s improved skill of %s at %s,current level is %d\n",me->query("id"),skill,ctime(time()),me->query_skill(skill,1)));
        return 1;
}
...
string select_skill(object me)
{
        mapping skills;
        string *skill_key,skill,*common_skill;
        int i;


        if( !mapp( skills = me->query_skills() ) )
                return "";
        if( !arrayp( skill_key = keys(skills) ) )
                return "";
        if( !(i=sizeof(skill_key)) )
                return "";

        if( DEBUG || (i > 40 && me->query("spi") > 40) )    //功夫太多
                return "unselected";

        skill = "";
        while(i--)
        {
                if( skills[skill_key[i]] > 230 || (string)SKILL_D(skill_key[i])->type()!="martial" )
                {
                        skill_key[i] = 0;
                        continue;
                }
        ...
        skill_key -= ({0});
        skill = skill_key[random(sizeof(skill_key))];
        return skill;
}
...
string *bonus = ({
        ...
        if( me->query("age") >= 80 )
        {
                command("say 这位"+RANK_D->query_respect(me)+"操劳了一生，坐下来先休息一下。");
                command("say 这些琐碎小事，还是让年轻人去做吧。\n");
                return 0;
        }
        ...
        if( base_name(ob) == "/obj/example/stone" )
        {
                ...
                i = random(100);
                if( i == 99 && me->query("task/task_count") < 30 ) i = random(100);
                if( DEBUG ) i = 99;

                //是否是合法生成的宝物？

                if( ob->query("task_serial") != query("task_serial") && !wizardp(me) )
                {
                        command("say 你这是哪里弄来的宝物？竟敢擅自私藏官家之物，你不怕满门抄斩吗？");
                        destruct(ob);
                        return 1;
                }

                //先生成宝物序列的断点
                if( ob->query("task_serial") != query("last_accept_serial")+1 || !query("last_broken_sn") )
                {
                        //收到的宝物中间已经有中断了。
                        set("last_broken_sn",ob->query("task_serial")-1);
                }

                //是否连续完成?
                if( me->query("last_task_serial") < query("last_broken_sn")  )
                {
                ...
                set("last_accept_serial", ob->query("task_serial") );
                me->set("last_task_serial", ob->query("task_serial") );
                destruct(ob);
               if( me->query("task/task_count") < 10 ) i = 51+random(37);

                // 发放奖励
                if( DEBUG ) me->set("task/task_count",50);
                if( i < 10 ) //特别奖励
                {
                        command("say 恭喜你!获得了我们的特别奖励!");
                        k = random(sizeof(bonus_key));
                        bonus_amount = to_int( bonus_base[k] * to_float(me->query("task/task_count")) / 2 );//yeung 2000-11-03
                    if(bonus_amount < 1) bonus_amount = 1;
                //给加个底线，总挖到0太不爽了，by masterall
                        b_amount = bonus_amount;
                        ...
                        me->save();
                        if( bonus_amount > 0 )
                                log_file("WABAO_BONUS",sprintf("%s gain %d points of %s at %s,now %d\n",me->query("id"),b_amount,bonus_key[k],ctime(time()),bonus_amount));
                        return 1;
                }
                ...
                if( i < 50 && i >= 10 ) //经验
                {
                        ...
                        k = base_exp * me->query("task/task_count") / 2; //yeung 2000-11-03
                        k = random(k) + k/2;
...
//                                me->set("combat_exp",6400000);
                        me->set("wabao_combat_exp",me->query("combat_exp"));
                        me->add("potential",k);
//                        if( (me->query("potential") - me->query("learned_points")) > 1000 )
//                                me->set("potential",me->query("learned_points")+1000);
                        me->add("score",1);
                        me->save();
                        return 1;
                }
                ...
                {
                        case 99:
                                command("say 恭喜你!获得了我们的特等大奖!");
                                message_vision("$n大声宣布：$N获得此次夺宝探险的特等奖励 - 由本人亲自指点功夫一次。\n",me,this_object());
                                skill = select_skill(me);
                                if( skill == "unselected" )
                                {
                                        me->set_temp("learn_from_yin",1);
                                        command("say 你的功夫太多太杂了，我都不知道教什么好，你自己选(select)一门功夫吧。");
                                }
                                else
                                if( skill != "" )
                                {
                                        message_vision("$n向$N仔细讲解「"+to_chinese(skill)+"」运用技巧，$N听着听着，若有所悟...\n",me,this_object());
                                        me->improve_skill(skill,100*me->query("task/task_count"));
                                        log_file("WABAO_BONUS",sprintf("%s improved skill of %s at %s,current level is %d\n",me->query("id"),skill,ctime(time()),me->query_skill(skill,1)));
                                }
                                else
                                {
                                        command("say 真可惜，你没有什么我可以指点的功夫。");
                                        command("say 我给你清除一次体内的不良状态吧。");
                                        me->clear_condition();
                                }
                                break;
                        case 98:
                                 paper = new("/d/qiankun/obj/xiandan");
                        ...
                        case 97:
                        case 96:
                                paper = new("/d/xueshan/npc/obj/xsword");
                                message_vision("$n将二等奖奖品："+paper->query("name")+"交给$N！\n",me,this_object());
                                paper->move(me);
                                break;
                        case 95:
                        case 94:
                                paper = new("/d/xueshan/npc/obj/xglove");
                                message_vision("$n将二等奖奖品："+paper->query("name")+"交给$N！\n",me,this_object());
                                paper->move(me);
                                break;
                        case 93:
                        ...
                        case 89:
                        case 88:
                                paper = new("/d/canyon/npc/obj/super_hat");
                                message_vision("$n将二等奖奖品："+paper->query("name")+"交给$N！\n",me,this_object());
                                paper->move(me);
                                break;
                        case 87:
...
int suitable_npc(object npc)
{
                return (
                                ...
                                strsrch(base_name(environment(npc)),"/newb/") == -1 &&
                                strsrch(base_name(environment(npc)),"/sky/") == -1 &&
...
int suitable_room(object room)
{
                return (
                                ...
                                strsrch(base_name(room),"/newb/") == -1 &&
                                strsrch(base_name(room),"/xixia/") == -1 &&
                                strsrch(base_name(room),"/sky/") == -1 &&
