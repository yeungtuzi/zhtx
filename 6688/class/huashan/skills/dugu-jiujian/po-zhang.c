...
int perform(object me, object target)
{
        ...
        seteuid(ROOT_UID);
        if( !me->is_fighting() )
                return notify_fail("「破掌式」只能在战斗中施用。\n");                                
        if( me->is_busy())
        ...
        || (string)ob->query("skill_type") != "sword")
                return notify_fail("你手中无剑，如何使用独孤九剑的「破掌式」。\n");     
        ...
        if( skill < 150 ) return notify_fail("你的「独孤九剑」招式不够娴熟，使不出「破掌式」。\n");                                   
        if( me->query_skill("strike",1)<150) return notify_fail("你的基本功不够扎实，无法使用「破掌式」。\n");
        
                 
        if( me->query("betrayer"))
            return notify_fail("你回想起以前在师门中所曾经受过的好处，再也无法使出独孤九剑来了。\n");
            
        if(!check_weapon(target,"strike"))
            return notify_fail("对手并非空手，你如何使用「破掌式」？\n");
        message_vision(HIY"$N使出独孤九剑「破掌式」，静气凝神，仔细观察$n的一举一动，试图找出$n的破绽.\n" NOR, me,target);
...
int do_fire(object me,object target)
{
        ...
        {
                message_vision(HIR"\n只见$N挺剑刺出，正中$n的最薄弱之处。$n手掌一凉，被$N长剑对穿而过！\n"NOR,me,target);
                target->set("force_factor",0);
                if( (damage = (damage-target->query_temp("apply/armor_vs_force"))) > 0 )
                {        
...
int do_prepare(object me,object target)
{
        ...
        if( left_busy > 3 )
        {
                ...
                {
                        tell_object(me,"对手并非空手，没法使用「破掌式」。\n");
        ...
        if( left_busy > 1 )             
        {
                if(check_weapon(target,"strike"))
                {
                        ...
                        ap  = (lvl*lvl)*lvl*mwx*mlx/900;//和自己sword等级，后天悟性灵性有关
                        dp  = target->query("combat_exp");//对方也是一样
                        if( random(ap+dp)>dp)
                                {
        ...
        if( left_busy > 0 )              
        {
                ...
                {
                        tell_object(me,"对手手上已经没有剑了，没法使用「破掌式」。\n");
                        me->delete("prepare_dugu");
                        me->add_temp("apply/defense",-100);
                        return 0;
                }
