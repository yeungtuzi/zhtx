...
void create()
{
         ...
         set("long",
                 ...
                 "他野心勃勃，准备一统武林，称霸天下\n"
                 "他手中的小楼一夜听春雨，与昔年威震天下的魔教教主的神刀名字相同。\n"
                 "而他本人据说也刀法通神，但是从来没有人见过。\n");
...
string ask_blade(object who)
{
	object obj = find_living("liu ruosong");
	if( objectp(obj)) 
	{
		if ( present( obj,environment(this_object()) ) )
		{
			message_vision(CYN"柳若松哼了一声。\n"NOR, environment());
			message_vision(CYN"柳若松说道：老夫行不更名，坐不改姓，正是神刀山庄庄主，柳若松！\n"NOR, environment());
		}
		
		return "？";
	}

	if (!this_player()->query_temp("weaponor5")) return "我没听说过。";
	if( (random(10) < 8) || is_fighting() ) return "你问这个干吗？\n";
	
	remove_call_out("dest");
	call_out("dest",1);
    return "哈哈哈哈哈哈哈哈，想不到我隐姓埋名这么多年，居然最后还是被识穿了。\n";
}
...
{	
	object liu;
	message("vision",
                RED "\n司空晓风眼中突然放出异样的光芒，缓缓道：是谁告诉你我以前的事情的？\n"
                "\n既然如此，老夫也没有必要再隐瞒了，不错，老夫就是当年名震天下的柳若松！\n"
                "\n言罢！将手中佩剑插回腰间，却从腰间抽出一把弯如新月的宝刀来握在手上。\n"NOR,environment(), this_object() );
	liu=new("/u/z/zephyr/liuruosong.c");
	liu->move(environment(this_object()));
