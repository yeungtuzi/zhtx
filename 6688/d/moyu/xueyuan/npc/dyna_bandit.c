...
void create()
{
	...
	if(random(2)>=1) 
	{
		set_name("蒙面人", ({get_rndid(),"bandit","dao"}));
		set("title", RED"强盗"NOR);
	}
	else
	{
		set_name("黑衣人", ({get_rndid(),"bandit","fei"}));
		set("title",RED"劫匪"NOR);
	}
	...
	set("dex", 25+random(30));
	set("int", 25+random(30));
	set("con", 25+random(30));
	...
	set("combat_exp", 600000+random(1000000));
	set("score", 1);
	set("block_all",1);
        setup();
	random_skills();
	if(random(10)>8) carry_rnd_equip(0,2);
	if(random(10)>8) carry_object("/d/moyu/obj/nineflower");
}
...
void random_skills()
{
	...
	set_skill("force", 80+random(60));
	set_skill("parry", 80+random(60));
	...
	set_skill(rdodge_skill,80+random(60));
	set_skill(rweapon_skill[0],80+random(60));
	set_skill(weapon_skill_type,80+random(60));
...
void greeting(object ob)
{
	...
	if (interactive(ob))
	{
		command("say 你这家伙站住，有什么值钱宝贝都给我留下！\n");
		me->set_leader(ob);
