...
HIC"“苍天不负我命，终于让我等到了这一天，好孩子，这把断魂砂今后就交给你了！”\n",NOR,
HIR"“一定要好好珍惜它，记住，‘镖在人在，镖亡人亡！’”\n",NOR,
HIG"“听说我的师兄唐糖后来归隐去了侠客岛，如果有机会的话将来你去找他，”\n",NOR,
...
void create()
{
        ...
        set("chat_msg_combat", ({
                ...
                (: exert_function, "quickpoison" :),
                (: exert_function, "unpoison" :),
        }) );
        ...
        create_family("唐门", 2, "无名前辈");
        set("title", HIB"乾坤教囚徒"NOR);
        set("nickname",HIR "逝去的辉煌，永世的神伤"NOR);
        setup();

        carry_object("/d/qiankun/obj/weapon/duanhun-sha")->wield();
}
...
void die()
{
       ...
       command("sigh");
       if(me->query_skill("mantian-huayu",1) > 199) 
       {
       me->remove_all_enemy();
       ob->remove_all_enemy();
        revive();
...
{        
        object weapon = new("/d/qiankun/obj/weapon/duanhun-sha");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("duanhun sha",who) )) 
                {
                weapon->move(who);
                }
