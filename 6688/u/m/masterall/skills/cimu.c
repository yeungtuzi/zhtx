...
int perform(object me, object target)
{
        ...
        if( !objectp(weapon = me->query_temp("weapon")) && weapon->query("id") != "xiuhua zhen" )
        ...
        if(judge(me,target)) {
               ...
               if(target->query_con()> random(skill_level/3)){//对方体质比较好，不至于一下给刺挂了。
               ...
               }else if (target->query_bln() > random(skill_level/3))  {//对方平衡的能力如何
               message_vision(HIR"\n但见红影闪处，似有一物向自己左目戳来！\n"+HIY"此刻$n已不及挡架，又不及闪避，只好侧头一让，但觉眉心一阵钻心的剧痛，\n"+HIB"幸好$N并未贯注全部真力，但被刺之苦楚已非蜂蛰蛇咬所能比了。\n" NOR, me, target);
               target->receive_wound("kee",me->query("force_factor")*4);
               target->receive_wound("gin",me->query("force_factor")*2);
               target->receive_wound("sen",me->query("force_factor")*2);
               COMBAT_D->report_status(me,target,1); 
 
               }else{ //这下可是结结实实给干上了。
               message_vision(HIG"\n猛听得$n大叫一声，兵刃落地，一个筋斗翻开了出去，"+HIW"\n双手按住右目，满地翻滚，连声惨嚎，这只眼睛已被$N刺瞎！！\n" NOR, me, target);
               target->receive_wound("kee",me->query("force_factor")*8);
               COMBAT_D->report_status(me,target,1); 
               target->start_busy(5);
               ...
               if( random(target->query_skill("force"))/2 > me->query_skill("sword")){ //内功强的兵器就飞了。
