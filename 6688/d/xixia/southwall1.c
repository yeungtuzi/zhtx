...
LONG
  ...
  "west" : __DIR__"southwall3",
  "east" : __DIR__"southwall2",
  "eastdown" : __DIR__"southgate",
  "enter" : __DIR__"southtower",
]));
	set("no_clean_up", 0);
	set("objects", ([
...
void alert(int where, object who)
{
	...
	if( !objectp(river = find_object(__DIR__"nhch")))
                if( !objectp(river = new(__DIR__"nhch")))
			return;
...
int action_bridge(string arg)
{
	...
	if (arg == "down") {
		if (bridge->query("exits/south") && road->query("exits/north"))
			...
			bridge->set("exits/south", __DIR__"southroad1");
