...
{      ob=this_player();
	...
	{
		 if((int)ob->query("marks/林平之")==0 && !query("crazy") )
	say( "林平之说道：“这位"+ RANK_D->query_respect(ob) 
+"，可曾见过家父林镇南？” \n");
	else
	{
	 ob->set("marks/林平之", 0);
         ob->set("marks/pixie1",1);
	 ob->set("potential", (int) ob->query("potential")+60+random(41));
	   call_out( "leave_stage", 1, this_player(), 1 );
 ...
 void leave_stage(object who, int stage)
{
	...
	return;
        ...
        room1= load_object(__DIR__"well");
        room1->set("marks/havebox",1);
