...
void check_trigger()
{
    ...
    if(!query("exits/enter") ) {
        if( !(room = find_object(__DIR__"hkduchuan")) )
            room = load_object(__DIR__"hkduchuan");
        if( room = find_object(__DIR__"hkduchuan") ) {
            if((int)room->query("yell_trigger")==0 ) {
                room->set("yell_trigger", 1);
                set("exits/enter", __DIR__"hkduchuan");
...
void check_trigger_fast()
{
    ...
    if(!query("exits/hkduchuan_fast") ) {
        if( !(room = find_object(__DIR__"hkduchuan_fast")) )
            room = load_object(__DIR__"hkduchuan_fast");
        if( room = find_object(__DIR__"hkduchuan_fast") ) {
            if((int)room->query("yell_trigger_fast")==0 ) {
                room->set("yell_trigger_fast", 1);
                set("exits/fastboat", __DIR__"hkduchuan_fast");
                room->set("exits/out", __FILE__);
                message("vision", "一叶扁舟缓缓地驶了过来，艄公将一块踏脚"
                    "板搭上堤岸，以便乘客\n上下。\n", this_object() );
                message("vision", "艄公将一块踏脚板搭上堤岸，形成一个向上"
...
void arrive()
{
    ...
    if( room = find_object(__DIR__"hkduchuan") )
    {
        room->set("exits/out", __DIR__"hkdukoub");
...
void arrive_fast()
{
    ...
    if( room = find_object(__DIR__"hkduchuan_fast") )
    {
        room->set("exits/out", __DIR__"hkdukoub.c");
        message("vision", "艄公说“到啦，上岸吧”，随即把一块踏脚板"
            "搭上堤岸。\n",room );
    }
