...
int main(object me, string arg)
{
	...
	if( me->query_temp("begging") )
        ...
        if( victim->query("id")=="saodi seng") return notify_fail("你不能向这样的长者乞讨！\n");
	...
	{
		me->set("gin", 0);
		return notify_fail("你太累了，没法乞讨！\n");
	}

	me->set("gin", me->query("gin") - 20);
        ...
        if ( ob->query("equipped") || ob->query("no_drop") || ob->query("no_get") )
                return notify_fail("这是那人的随身家伙，肯定不会给你。\n");
	if ( mapp(ob->query("weapon_prop"))) 
		return notify_fail("武器乃护身之物，肯定不会给你。\n");
	if ( mapp(ob->query("armor_prop")) ) 
		return notify_fail("要人身上的衣服？太过分了吧！\n"); 
	if ( mapp(ob->query("no_beg"))) 
		return notify_fail("这件东西不会给你。\n");
        sp = ((int)me->query_skill("begging")-20) * 15 + (int)me->query("kar") * 5
                - (int)me->query("begger") * 2;

        if( sp < 1 ) sp = 1;
        dp = (int)victim->query("gin");

        tell_object(me, "你装出可怜巴巴的样子，慢慢地向" + victim->name() + "走�去，伸出双手，想�"
