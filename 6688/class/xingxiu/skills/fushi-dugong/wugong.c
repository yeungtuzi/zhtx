...
void create()
{
        set_name(HIR"赤精"+HIY"蜈蚣"NOR, ({ "chijing wugong"}));
        set("long", HIC"一条大蜈蚣，全身闪光，头上凸起一个小瘤，与寻常蜈蚣大不相同。\n"NOR);
        setup();
}
...
void invocation(object who)
{
 ...
 //       set("verbs", ({ "poke" , "claw" }) );
        ...
        message_vision(HIG"忽听得草丛中瑟瑟声响，绿草中红艳艳地一物晃动，却是一条大蜈蚣蜿蜒而至，\n蜈蚣闻到木鼎中发出的香气，径身游向木鼎，虎视耽耽的徘徊在$N的四周。\n"NOR, who );
        i = sizeof(enemy);
        while(i--) {
        ...
        if( enemy[i]->query_condition("wugong_poison") ) return;
        ...
        message_vision(HIG"$n躲闪不及，被喷了个满脸花，顿时哭天抢地般嘶嚎了起来 ！\n"NOR, me,enemy[i]  );
                enemy[i]->apply_condition("wugong_poison", 20+random(20));
        }
        else       
        {
