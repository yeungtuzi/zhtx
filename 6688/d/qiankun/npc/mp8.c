...
void create()
{
        ...
        create_family("晚月庄", 2, "无名前辈");
        set("title", HIB"乾坤教囚徒"NOR);
        set("nickname",HIR "逝去的辉煌，永世的神伤"NOR);
        setup();

        carry_object("/d/qiankun/obj/weapon/mo-bian")->wield();
}
...
void die()
{
       ...
       command("sigh");
       if(me->query_skill("snowwhip",1) > 199) 
       {
       me->remove_all_enemy();
       ob->remove_all_enemy();
        revive();
...
{        
        object weapon = new("/d/qiankun/obj/weapon/mo-bian");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("mobian pianlong",who) )) 
                {
                weapon->move(who);
                }
