...
void create()
{
           set_name("春风醉" , ({"chun fengzui", "rio"}));
	set("title",HIR "日"+HIB"月"+HIY "神教" NOR + GRN+ tang+ NOR +"堂主");
	
	set("long", @DESC
    她是日月神教十长老之首独孤峰的得意女弟子。
她为神教的复兴曾立下了大功，被举为八堂主之列。
但她的心中却并不愉悦，此刻她又想到了什么呢？
梦回梦醒此生......
DESC
);
	set("nickname",MAG"回梦魔女"NOR);
	set("gender", "女性");
	set("age", 24);
...
if(this_player()->query("whichtang")!=0&&this_player()->query("whichtang
")!=tang)
	 {
	   write( RANK_D->query_respect(this_player()) +
                "你不是本堂弟子,请回吧\n");
	...
	this_player()->set("whichtang",tang);
        this_player()->set("title",HIR "日"+HIB"月"+HIY "神教" NOR + GRN
 +tang+ NOR
+"弟子");
