...
{
        int skill,skill1,preparetime;
        object ob;
        seteuid(ROOT_UID);
        if( !me->is_fighting() )
                return notify_fail("「破剑式」只能在战斗中施用。\n");                                
        if( me->is_busy())
        ...
        if(skill< 220){preparetime=5;}
        if(skill>= 220 && skill<250){preparetime=4;}
        if(skill>= 250 && skill<300){preparetime=3;}
        if(skill>= 300 && skill<400){preparetime=2;}
        if(skill>= 400){preparetime=1;}
        
        if( skill < 150 ) return notify_fail("你的「独孤九剑」招式不够娴熟，使不出「破剑式」。\n");                                   
        if( me->query_skill("sword",1)<150) return notify_fail("你的基本功不够扎实，无法使用「破剑式」。\n");
        ...
        if(!check_weapon(target,"sword")) 
            return notify_fail("对手手中并没有剑，你如何使用「破剑式」？\n");
         
        message_vision(HIY"$N使出独孤九剑「破剑式」，静气凝神，仔细观察$n的一举一动，试图找出$n的破绽.\n" NOR, me,target);
        me->add_temp("apply/defense",100);
        me->set_temp("prepare_dugu",preparetime);        
        me->start_busy(
...
int do_prepare(object me,object target)
{
        ...
        if( left_busy > 3 )
        {
                if(check_user(me,target))
                {
                        ...
                        {
                        tell_object(me,"对手手上已经没有剑了，没法使用「破剑式」。\n");
