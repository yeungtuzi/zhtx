...
{	
	...
	if (me->is_fighting() || wizardp(ob) || ob->query("combat_exp")<10000 )
		return;
...
void pretending(object ob)
{
	...
	switch( random(3) ) {
		case 0:
                       ...
                       set_temp("apply/id", ({ob->query("id")}));
		       set_temp("apply/name",({ob->query("name")}));
		       set_temp("apply/title",({ob->query("title")}));
