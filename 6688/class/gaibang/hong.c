...
*/
        ...
        command("eat ji");
        if (!(myfam = this_player()->query("family")) || myfam["family_name"] == "丐帮" || myfam["family_name"] == "少林派" )
        {       
		if( !me->query("betrayer") )        	
		{
			command("say 既然你如此孝顺，我就教你这招「天下无狗」。");
	                if (me->query_skill("dagou-bang", 1) >= 120)
        	        {
                	        say("你听了洪七公的讲解，学会了这一招。\n");
                        	me->set("skill_mark/dagou-bang",1);
	                        return 1;
        	        }
	                else
        	        {
                	        say("由于你的打狗棒法等级不够，对洪七公的讲解没有听懂。\n");
                        	return 1;
	                }
	        }
	        else
	        {
	        	command("say 打狗棒法乃本帮镇帮之宝，非忠心耿耿之弟子，不能传授。");
	        	say("洪七公吞了吞馋涎，还是摇了摇头。\n");
	        	return 0;
	        }	
        }
