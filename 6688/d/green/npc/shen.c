...
void create()
{
        ...
        set("vendor_goods", ([
                __DIR__"obj/newhammer": -1,
                __DIR__"obj/ricewine": -1,
		"/obj/toy/rope":-1,
        ]) );
        setup();
...
int list_item()
{
        ...
        write("每把 50 文钱. 墙角摆着一些铁锤,柜台上摆著一块木牌, 上面写著:\n");
...
int accept_object(object who, object ob)
{
	...
	{
		item = new(__DIR__"obj/jade");
		item->move(this_object());
		who->delete_temp("drunk");
		command("give jade to "+(string)who->query("id"));
		return 1;
	}
	if (ob->value() == 100) 
	{
		item = new("/obj/drug/slumber_drug");
		item->move(this_object());
