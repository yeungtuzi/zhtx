...
// Data: 1997.10.17         
// The latest changed by ysgl (1997.10.31)
...
void create()
{
	...
	set("vendor_goods", ([
		__DIR__"obj/cloth1.c" : 1,
		__DIR__"obj/cloth2.c" : 1,
                __DIR__"obj/cloth3.c" : 1,
                __DIR__"obj/cloth4.c" : 1,
                __DIR__"obj/cloth5.c" : 1,
	]));
...
int do_ding(string arg)
{
	...
	newfile = replace_string( newfile, "订做的", cname);
        ...
        newfile = replace_string( newfile, "xkx", id);
//	filename = DATA_DIR+"login/" + id[0..0] + "/" + id + "/" + id +sprintf("%d",time()) + ".c";
	filename = "/open/cloth/" + id +sprintf("%d",time()) + ".c";
	if( !write_file(filename, newfile, 1))
		return notify_fail(RED"柳云天一脸沮丧的说：“对不起，无法完成。”\n"NOR);
	ob = new(filename);
	if(ob->move(me)){
	gold->add_amount(-1);
