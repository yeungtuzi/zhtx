...
int exert(object me, object target)
{
    ...
    me->start_busy(4); // moved to here by Elon.
        ...
        ap = me->query("combat_exp")* me->query_per()/30;
        ...
        if( damage < 1 ) {
                message_vision(HIG"但是$N对$n的注视视若无睹....。\n"NOR, target, me);
                return 1;
        }
 
        target->receive_damage("gin", damage);
        if( random(skill + (int)target->query("cps") * 2) >(int)target->query("cps") * 2 )
                target->receive_wound("gin", damage);
        message_vision(HIB"$N被$n的目光所摄，不自禁地打了个寒噤。\n"NOR, target, me);
