...
void init()
{
	...
	//  最多完成三次
	who = this_player();
	if( (j=(int)(who->query("marks/银叶_credit")) < 3) && (who->query("marks/银叶_success")))
	{
		//第一次给奖励
		who->delete("marks/银叶_success");
		who->add("marks/银叶_credit",1);
		if( (j=(int)(who->query("marks/银叶_credit"))) ==1 )
		{
			tell_object(who,"你由于成功地解决了这个悬案而提高了自己的武林声望！\n");
			who->add("wlshw",5);
		}
		else if( (j=(int)(who->query("marks/银叶_credit"))) ==3 )
		{
			tell_object(who,"胡青牛感激地说："+RANK_D->query_respect(who)+"解开了老朽多年的一桩心事，大恩不敢言谢，\n
				我手头还有多年炼制的一点药材，略表寸心。\n");
			yao = new(__DIR__"obj/jiuzhuandan");
			yao->move(who);
		}
		else if( (j=(int)(who->query("marks/银叶_credit"))) ==2 )
		{
			if( present("wang nangu",environment(this_object())) )
			{
				tell_object(who,"\n\n王难姑感激地说：老太婆这里还有一本毒经，是我多年心血，就送于" + RANK_D->query_respect(who) + "，万望笑纳。\n");
				jing = new(__DIR__"obj/dujing");
				jing->move(who);
			}
		}
		who->add("combat_exp",random(1000));
	}
}
