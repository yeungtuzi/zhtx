...
void create()
{
	set_name(GRN"兵器贩子"NOR, ({ "weaponor","bingqi fanzi"}) );
	set("long",BLK@LONG
...
void slash(object victim, object player)
{
		...
		if (flag)
		{
			...
			message("sound", "\n"+name()+CYN""+HIW"深深地吸一囗气，真力迸发，发出一声惊天动地的巨吼！！\n"NOR, environment());
		...
		//		message("vision", HIW"\n只见一道雪亮的白光划过" +victim_name+ HIW"的胸膛\n"NOR, environment());
				...
				case 2:	victim->start_call_out( (: call_other, __FILE__, "remove_effect", victim :), 50);
						victim->add_temp("apply/armor", -random(500));
				...
				case 3:	tell_object(victim, YEL"你只觉出跸刀滑过你的皮肤，居然丝毫不觉疼痛，甚至还有种很享受、很舒服的感觉。\n"); 
						...
						{
							if (random(100)>75)
                                victim->apply_condition("tm_poison", random(10)+victim->query_condition("tm_poison"));    
							if (random(100)>75)
								victim->apply_condition("rose_poison", random(10)+victim->query_condition("rose_poison"));    
 							if (random(100)>75)
                               victim->apply_condition("ninemoonpoison", random(10)+victim->query_condition("ninemoonpoison"));    
							if (random(100)>75)
                                victim->apply_condition("blockweb", random(10)+victim->query_condition("blockweb"));    
							if (random(100)>75)
                                victim->apply_condition("snake_poison", random(10)+victim->query_condition("snake_poison"));    
							if (random(100)>75)
                                victim->apply_condition("zhang_qi", random(10)+victim->query_condition("zhang_qi"));    
							if (random(100)>75)
                                victim->apply_condition("xiezi_poison", random(10)+victim->query_condition("xiezi_poison"));    
							if (random(100)>75)
                                victim->apply_condition("super_poisons", random(10)+victim->query_condition("super_poisons"));    
							if (random(100)>75)
                                victim->apply_condition("xx_poison", random(10)+victim->query_condition("xx_poison"));    
							if (random(100)>10)
                                victim->start_busy(5+random(10));    
							if (random(100)>75)
							{
                                victim->apply_condition("doom", 3+random(5) );    
