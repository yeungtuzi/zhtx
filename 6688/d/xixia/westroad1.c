...
void create()
{
	set("short", "大道");
	set("long", @LONG
从此西出中兴府，是一条颇为凄凉的大路，路两旁是一望无际
的大草原。放眼望去，四周皆是一遍枯黄色。不时一阵狂风卷来，
吹得你连眼睛都睁不开，风声中还隐隐挟带着几声声牛马嘶鸣。
LONG);
	set("outdoors", "xixia");
	set("exits", ([ /* sizeof() == 1 */
		"west" : "/d/road/jiayuguan",
		"east" : __DIR__"westbridge",
...
int do_jump(string arg)
{
	string skill, desc, weapon_type;
        ...
        if( !objectp(wall = find_object(__DIR__"westwall1")))
                if( !objectp(wall = new(__DIR__"westwall1")))
                        return 0;
	...
	if (objectp(weapon))	weapon_type = weapon->query("type");
