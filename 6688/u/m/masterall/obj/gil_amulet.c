...
/*
        if (wizardp(me))
        {
                tell_object(me,"your effective gil_flag is "+player_gil_flag+"\n");
                tell_object(me,"zhaohuan_gil is "+me->query_condition("zhaohuan_gil")+"\n");
        }
*/
        if(!me) return ;
        ...
        {                       
                if( me->is_fighting() && (gil) )
                {
                        //只在战斗之初判断一次是否召唤基加美修
                        gil = 0;
                        if( random(100)>0 && !me->query_condition("zhaohuan_gil") && random(player_gil_flag) > 0)
                        {       
                                me->apply_condition("zhaohuan_gil", 10);

                                if (wizardp(me)) tell_object(me,"zhaohuan_gil set to "+me->query_condition("zhaohuan_gil")+"\n");
