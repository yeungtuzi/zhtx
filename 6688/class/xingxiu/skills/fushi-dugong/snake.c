...
void create()
{
        set_name(HIC"青冠"+HIW"巨"+BLK"蟒"NOR, ({ "qingguan jumang"}));
        set("long", HIC"一条白身黑章的大蟒蛇，蟒蛇头作三角形，头顶上高高生了一个凹凹凸凸的肉瘤。\n"NOR);
        setup();
}
...
void invocation(object who)
{
        ...
        set_name(HIC"青冠"+HIG"巨蟒"NOR, ({ "qingguan jumang"}));
        set("long", HIC"一条白身黑章的大蟒蛇，蟒蛇头作三角形，头顶上高高生了一个凹凹凸凸的肉瘤。\n"NOR);

        set("nickname", HIB"附骨之噬"NOR);
        set("race", "野兽");
 ...
 //       set("verbs", ({ "poke" , "claw" }) );
        ...
        message_vision(HIG"听得草丛中瑟瑟声响，杂着一股中人欲呕的腥臭，一条白身黑章的大蟒蛇蜿蜒游至，\n闻到香料及木鼎气息，顺从的守护在$N的周身四处。\n"NOR, who );
        i = sizeof(enemy);
        while(i--) {
        ...
        if( enemy[i]->query_condition("snake_poison") ) return;
        message_vision(HIB"\n$N"+HIB"头部的毒囊瞬间鼓胀，一股强烈的腥臭之气向$n激射而出！\n"NOR, me,enemy[i]  );
        if( random(me->query("combat_exp")) > (int)enemy[i]->query("combat_exp")/2 )
        {
        message_vision(HIG"$n躲闪不及，被喷了个满脸花，顿时哭天抢地般嘶嚎了起来 ！\n"NOR, me,enemy[i]  );
                enemy[i]->apply_condition("snake_poison", 20+random(20));
        }
        else       
        {
