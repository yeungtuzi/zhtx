// xiantian-gong.c 先天功
// Made by hacky (1997.10.18)

inherit FORCE;
#include <ansi.h>

string *absorb_msg = ({
        "$N的劲气竟奇迹地由阳刚化作阴柔，像风送落叶般将攻势一一化解。\n",

        "$N激发出一股无坚不璀的真气，让对方简直无可乘之机。\n",

        "$N的先天劲气似若长江大河，绵绵无尽，丝毫没有衰竭之象。\n",

        "$N凝立不动，凝神内视，对对手的伤害无动于衷，好像那一击根本不是打在自己身上。\n", 

        "$N容颜肃稷严厉，双目精光电闪，令人呼吸困难的惊人气劲陡然间膨胀了长许空间。\n",

        "$N的气劲像乾布吸水般一点一点的扩散，瞬间将攻势化于无形。\n",
        });



int valid_enable(string usage) { return usage == "force"; }

int valid_learn(object me)
{       
        int nt=   (int)me->query_skill("taoism", 1);
        int lvl = (int)me->query_skill("xiantian-gong", 1);
        if ( me->query("gender") == "中性")
                return notify_fail("你无根无性，阴阳不调，如何领会先天功？\n");
        if ( me->query("class") == "bonze" )
                return notify_fail("先天功追求的乃是自然之道，于佛家六根清净之意有碍，"
                        +RANK_D->query_respect(me)+"欲修此功，如何堪破世间百情？\n");
        if ((int)me->query_skill("force", 1) < 20)
                return notify_fail("你的基本内功火候还不够。\n");
        if ( nt-49 <= lvl && lvl<201)
                return notify_fail("你的道家心法修为不够，无法领会更高深的先天功。\n");
        return 1;
}

int practice_skill(object me)
{
        return notify_fail("先天功只能用学(learn)来增加熟练度。\n");
}

int bounce_ratio()
{
        return 60;
}

string exert_function_file(string func)
{
        return __DIR__"xiantian-gong/" + func;
}

string query_force_type() {
        return "道家内功";
}

varargs string  query_absorb_msg(string limb)
{
       return absorb_msg[random(sizeof(absorb_msg))];
}

mapping level_req(int level) {

        return( ([
                "force":20,     
        ]) );
}

int query_faith_req(int skill) {
        return 10;
}

