...
int main(object me, string arg)
{
	...
	if( !objectp(obj = present(arg, env)) || living(obj) )
		return notify_fail("你附近没有这样东西。\n");
          ...
          if( obj->is_character() && !obj->is_corpse() && !obj->query("taskkiller")&&

		// if user didn't set can_get  (nihao)
                ( !obj->query("env/can_get")))

		return notify_fail("看清楚点，那是活物！\n");
...
/*
	...
	if( obj->is_character() && environment(me)->query("no_death") )
		return notify_fail("这个房间禁止背负生灵。\n");
*/		
