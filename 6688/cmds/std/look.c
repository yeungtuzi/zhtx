...
//changed by tlang so can look what a person look like.
	if( (string)obj->query("race")=="人类"
	&&	intp(obj->query("age")) )
	{
		str += sprintf("%s看起来约%s多岁，\n"+"%s%s\n"+"这时候%s%s\n", pro,chinese_number(obj->query("age") / 10 * 10),
pro,per_status_msg((int)obj->query_per(), obj->query("gender"),obj->query("age")), 
pro,bel_status_msg((int)obj->query("bellicosity"), obj->query("gender")),obj->query("age") );
...
int look_room_item(object me, string arg)
{
	...
	if( stringp(msg = env->query("item_desc/" + arg)) ) {
		write(msg);
		return 1;
	}

	if( msg = env->query("exits/" + arg) ) {
		env = find_object(msg);
		if( !env ) {
			call_other(msg, "???");
			env = find_object(msg);
		}
		if( env ) look_room(me, env);
		else write("你只看到灰蒙蒙地一片黑暗，最好不要走过去，请巫师处理。\n");
...
{
	object weapon;
	string skill_type,parry_type,attack_skill;
	int attack_points;
	mapping prepare;
...
//	write("ap:"+attack_points);
        return  tough_level(attack_points/300+1);
//现在都太强了，还是改改样子，比较切合实际。by masterall
}                       
...
string getdam(object me, object obj)
{
	...
	level += obj->query("force")/60;
	level += obj->query("max_force")/30;
//	write("damage level:"+level);
//        level = level / 100;
//现在都太强了，还是改改样子，比较切合实际。
        level = level / 200;
