...
int perform(object me, object target)
{
        ...
        if( me->query("gin") <= skill )
                return notify_fail("你的精不够使用绊字诀！\n");


        weapon = me->query_temp("weapon");

        if( me->query_skill("staff") < 200 ) { hits=3; }
        else if( me->query_skill("staff") < 250 ) { hits=4; }
        else { hits=5; }

                if( random(2)) {
                        message_vision(CYN "突然$N"+ weapon->name()+CYN"挥出，以打狗棒的「绊」字诀着地扫去，有如长江大河，绵绵而至。\n\n" NOR, me, target); }
                else {
                        message_vision(HIY "$N"+ weapon->name()+HIY"伸出，施展打狗棒法的「绊」字诀，连环钩盘，虽只一个「绊」字，中间却蕴藏着千变万化。\n\n" NOR, me, target);}


        for(i=0;i<=hits;i++) {
                if (random(me->query("combat_exp")) * me->query_skill("staff") > (random((int)target->query("combat_exp")) * target->query_skill("dodge") * (1-(((ban_count * 2)+50)/100)))) {
                                        msgsuccess = ({
                                                "$n纵身稍慢，被"+ weapon->name()+WHT"在左胫上一拨，右踝上一钩，扑地倒了。",
                                                "$n立足不住，被"+ weapon->name()+WHT"绊了一交，仰天摔倒。",
                                                weapon->name()+WHT"腾的一下，$n登时措手不及，被绊得一交摔倒。",
                                                weapon->name()+WHT"猛然翻转，击中$n的脚胫。$p立足不稳，一个踉跄，扑地跌倒。"
                                        });
...
//                          me->start_busy(1 + random(2));
                        ...
                        //target->add("gin",-ban_count * ndamage);
                             target->add("kee",-me->query_skill("staff")/2);
            ...
            if (i < hits) {
                                        msgloop = ({
                                                "$n刚跃起，尚未落地，那"+ weapon->name()+HIY"又是一挑一打。",
