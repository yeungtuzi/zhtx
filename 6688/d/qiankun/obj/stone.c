...
int do_break()
{
         object me,wan,wan2,lu,cha,dan,dan2,shi;
         int luck;
         wan=new("/d/moyu/obj/nineflower");
         wan2=new("/class/taohua/obj/nineflower");
         lu=new("/d/qiankun/obj/nineflower");
         cha=new("/d/qiankun/obj/shenxiancha");
         dan2=new("/d/mingjiao/npc/obj/dan");
         dan=new("/d/mingjiao/npc/obj/jiuzhuandan");
         ...
         message_vision(HIY"$N凝聚真气，将全身的功力汇集于双臂，爆喝一声，那块石头立即零七八碎的裂成好几瓣。\n与此同时，$N居然发现其中暗藏着一样东西，黑暗中也看不清楚是什么，随手就把它拾了起来。\n"NOR,me);
         me->add("force",-1000);

         if( luck > 58 ){ 
//                     dan->move(me);
         }else if( luck > 50 && luck < 53){
//                     dan2->move(me);
         }else if( luck > 45 && luck < 51){
                     lu->move(me);
         }else if( luck > 35 && luck < 41){
                     cha->move(me);
         }else if( luck > 25 && luck < 36){
                     wan->move(me);
                     wan2->move(me);
         }else {
                     shi->move(me);
