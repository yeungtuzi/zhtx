...
void create()
{
        set_name(YEL"纵横"WHT"兵器谱"NOR, ({ "bingqi pu" , "zongheng bingqipu", "bingqipu", "amulet", "pu" }));
     set_weight(50);
     set("unpawnable",1);
...
温柔：原为神兵利器灵空剑，由铸剑大师邵空子炼成，未几邵大师请一名宿相此宝剑，
        那名宿却认为是一柄凶剑，剑身上光纹四射如火，主大凶，佩带都必定招
        致不详，甚至会有家破人亡之祸。后来此剑被邵空子所毁，再用残剑的余铁炼
	成一柄其薄如纸的薄刀，后来神剑应无物以一本剑谱将之换走，又传给了他的
	爱徒世袭一等侯狄青麟，狄青鳞曾以之杀害河朔大侠万君武，由于又薄又快，
	杀人后竟不见血迹。此刀有影无踪、有形无质、其快如电、柔如发丝。在杀人时，
	就像情人的拥抱。因此，名唤“温柔”。
...
LONG NOR
                        ...
                        set("unit", "册");
            set("value", 0);
            set("armor_prop/damage",290);
            set("armor_prop/armor",150);
            set("armor_prop/armor_vs_force",210);
    ...
    int i, all, player_gil_flag, heal_kee, maxkee, effkee, kee;
        ...
        {       
                if( me->is_fighting() && (gil) && !me->query_condition("zhaohuan_weaponor" ) )
                {
                        //只在战斗之初判断一次是否召唤基加美修
                        gil = 0;
                        if( random(100)>75 && random(player_gil_flag) > 800)
                        {       
                                ...
                                all = sizeof(enemy);
                                if( !all ) return;
                                obod=new(__DIR__"gilgamesh_4");
                                obod->move(environment(me));
                                
								me->apply_condition("zhaohuan_weaponor", 10+random(15) );
...
int do_feel(string arg)
{
	...
	int value = 20 * (player_gil_flag - 1000) /  player_gil_flag;
	int w = 80 * (player_gil_flag - 1500) /  player_gil_flag;
