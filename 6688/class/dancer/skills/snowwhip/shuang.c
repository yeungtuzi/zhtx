...
int perform(object me, object target)
{
        ...
        mydodge= (int)me->query_skill("stormdance",1);
        mneili = (int)me->query("force");
        mexp = (int)me->query("combat_exp");
        if(mydodge<221){mybusy=3;}
        if(mydodge<281&&mydodge>220){mybusy=2;}
        if(mydodge>280){mybusy=1;}
        msg = "";
        if(!objectp(weapon)||(string)weapon->query("skill_type")!="whip")
                return notify_fail("不拿鞭子怎使「月冷如霜」?\n");
        if(me->query("family/family_name")!="晚月庄")
     ...
     if (random(mexp)>(int)(texp/10) && random(mneili) > (int)(tneili/3)&&
        ...
        {
             msg = HIW"$n只觉得一股寒气象化作千百枚细针一样，穿体而过！\n"
     ...
     COMBAT_D->report_status(me,target,1);   
           ...
           msg = HIY"$n忙运功与之相抗，深厚的内力将$N的漫天鞭影和无边的寒意都激散了。\n\n"NOR;
           message_vision(msg, me, target);
        }
     ...
     {
             msg = HIW"$n被鞭风一荡，头脑都似乎被冻僵了，不知该如何去招架这似雷、似电又似风的鞭法！\n"
     ...
     COMBAT_D->report_status(me,target,1);   
          ...
          msg = HIG"$n却心如止水，以快制快，身形飘忽间就脱出了$N这凌厉的攻势。\n\n"NOR;
          message_vision(msg, me, target);
        }
          ...
          {
                msg = HIR"$n果不其然，以为$N已为己所伤，急忙探身上前抢攻。\n"
          ...
          msg = HIC"$n早就看破了这招的虚实，假意前冲，却在$N出招的一瞬间回手反攻一招！\n\n"NOR;
              ...
              if( random(me->query_cps()+me->query_skill("dodge")) < target->query("cor")*3 ){
          message_vision(msg, me, target);
          COMBAT_D->do_attack(target, me, tweapon);
              }else{
     message_vision(HIM"$N一招失手，随即展动身形，如孤雁般飞起，堪堪避开了$n的这反戈一击！！\n" NOR,me,target);
              }
        }
