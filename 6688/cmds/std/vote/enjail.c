...
int vote(object me, object victim)
{
  ...
  if ((reason = (int)victim->query("vote/reason"))>0 && (reason!=V_ENJAIL))
  {
  	return notify_fail("要把当前的表决完成以后才可以提新的动议。\n");
  }

/*
  if( victim->query("enjail_on") ) 
  {
  	return notify_fail(victim->name()+"已经在监狱里了。\n");
  }

*/
  if (reason <= 0)
  {
  	victim->set("vote/reason", (int)V_ENJAIL); 
  }
  ...
  {
	shout( HIG "【表决】"+me->name()+"投票将" +victim->name()
			+"送入监狱。"+victim->name()+"被送入监狱了！\n" NOR);
	write( HIG "【表决】"+me->name()+"投票将" +victim->name()
			+"送入监狱。"+victim->name()+"被送入监狱了！\n" NOR);
			
	victim->apply_condition("vote_clear", -10);
        victim->move("/d/wizard/jianyu");
  } 
