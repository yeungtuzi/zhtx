...
HIC"“苍天不负我命，终于让我等到了这一天，好孩子，这把暗灵今后就交给你了！”\n",NOR,
HIR"“一定要好好珍惜他，剑在人在，剑亡人亡！”\n",NOR,
YEL"“听说我的师兄地狱圣火后来归隐去了侠客岛，如果有机会的话将来你去找他”\n",NOR,
...
void create()
{
        ...
        set_skill("gouyee", 220);
        set_skill("taosword", 220);
        set_skill("taoism", 220);
        set_skill("necromancy", 220);
        ...
        create_family("茅山派", 2, "前辈");
        setup();

        carry_object("/d/qiankun/obj/weapon/anling")->wield();
}
...
void die()
{
       ...
       command("sigh");
       if(me->query_skill("spells",1) > 199) 
       {
       me->remove_all_enemy();
       ob->remove_all_enemy();
       "/cmds/imm/full"->main(ob,"");
...
{        
        object weapon = new("/d/qiankun/obj/weapon/anling");       
        object ob;        
        ...
        if( ++stage < sizeof(msg_weapon) ) 
        {
                ...
                if (who->query_temp("mpweapon_win") && !objectp(ob = present("an ling",who) )) 
                {
                weapon->move(who);
                }
