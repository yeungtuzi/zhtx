// dongsw add for zh2 system
#include <ansi.h>
#include <net/dns.h>

inherit F_CLEAN_UP;
#define PREFIX(ob)    (interactive(ob) ? (query_idle(ob) > 120 ? HIM "+" NOR : " ") : HIG "*" NOR)
...
mixed main(object me, string arg, int remote)
{
        ...
        str =  HIC" " + MUD_NAME +  " \n"NOR;
             str += "────────────────────────────────────────\n";
          ob = filter_array(objects(), (: userp($1) :));
        if (opt_party != "(null)")
        ...
        if( opt_long || opt_wiz ) {
            ...
            if( opt_wiz && !wiz_level(list[i]) ) continue;
                ...
                while( i-- ) {
                        ...
                        if (!wiz_level(list[i]) && fname != list[i]->query("family/family_name")) {
                            ...
                            str += sprintf( YEL"%-10s"NOR , (fname?fname:"普通百姓") + "：");
                            count = 1;
                        }
           ...
           str += "────────────────────────────────────────\n";
        str = sprintf("%s共有 %d 位玩家连线中，系统负担：%s\n"+HIG"*"NOR+" 表示"+HIG" 断线 "NOR+"中 "+HIM"+"NOR+" 表示"+HIM" 发呆 "NOR+"中", str, ppl_cnt+cnt,
                 query_load_average());
