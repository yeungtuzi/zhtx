...
int exert(object me, object target)
{
        ...
        if (( random(sp) > random(dp) ) || !living(target) )
	{
		...
		target->add("force", -1*((int)me->query_skill("beiming-shengong", 1) + target->query("force_factor")) );
            if ( target->query("force") <1 ) target->set("force",0);
		me->add("force", (int)me->query_skill("beiming-shengong", 1) + target->query("force_factor") );
