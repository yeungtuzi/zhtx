//OBJ:/d/xueshan/npc/obj/xsword.c
//By masterall for 小楼一夜听春雨

#include <weapon.h>
#include <ansi.h>

inherit F_AUTOLOAD;
inherit SWORD;
//inherit F_UNIQUE;

int query_autoload()
{
        return query("weapon_prop");
}

void create()
{
        set_name(MAG"小楼"+BLK"一夜"+HIG"听"+HIB"春雨"NOR, ({ "xiao lou", "jian" }) );
        set_weight(1);
        set("owner","no_owner");
        set("marks/寒于",1);
        set("no_drop",1);
        set("no_get",1);
        set("unpawnable",1);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("unit", "把");
                set("long",
                        "这是当年魔教教主使用的一把利刃，冷冷的剑身散发出逼人的杀气。\n不知道这把剑饮了多少人的血，隐隐的有一种殷红色的光芒。\n");
                set("value", 0);
                set("rigidity",10000);
                set("material", "sapphire");
                set("wield_msg",
                        HIR"$N「刷」地一声抽出了$n，天地瞬间为之变色，四周弥漫着逼人的杀气。\n"NOR);
                set("unequip_msg",  "$N将手中的$n插入腰间的剑鞘。\n");
                set("weapon_prop/attack", this_player()->query_skill("fuliu-jian",1)/7);   
                set("weapon_prop/dodge", this_player()->query_skill("fall-steps",1)/4);       
        }
    init_sword(300);
       setup();
}

int wield()
{
        if( this_player()->query_skill("fuliu-jian",1) < 250)
        {
                return notify_fail("你使不动这件兵器。\n");
        }
        return ::wield();
}

mixed hit_ob(object me, object victim, int damage_bonus)
{
          int skill;
          skill=this_player()->query_skill("fuliu-jian",1);
               if (damage_bonus < 0 ) return 0;
          if(skill>random(399))
     //   if(random(3)==0)
        victim->receive_wound("kee",damage_bonus/3);
        return this_object()->name()+ HIR "刺中了$n的$l，疮口登时鲜血狂飙，$n发出的连声惨嚎连天地鬼神都为之胆寒！\n" NOR;
}


