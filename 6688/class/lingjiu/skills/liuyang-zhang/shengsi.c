...
int start_hehe(object me, object target)
{
        ...
        if( random(ap + dp) > dp ){
                ...
                {
                message_vision(HIC"\n$N大腿"+HIG"“伏兔穴”"+HIC"上一阵阵寒意袭来，动作顿时有些不大灵光，好似被冻僵了一般麻木。\n"NOR, target);
                target->set_temp("last_damage_from",me);
                target->apply_condition("sheng_si", 81*60);  
                target->update_condition("sheng_si");
                target->start_busy(5);
                ...
                {
                message_vision(WHT"\n$N上臂"+HIG"“天泉穴”"+WHT"好像被什么东西斫了一下，顿时半个膀子耷拉下来，变得绵软无力，贯注不得办点真力！！\n"NOR, target);
                target->set_temp("last_damage_from",me);
                target->apply_condition("sheng_si", 81*60);  
                target->update_condition("sheng_si");
                ...
                {
                message_vision(HIM"\n$N觉得后颈"+HIG"“天柱穴”"+HIM"微微一凉，还不断在纳闷，“今天好像没下雨啊！”\n"NOR, target);
                target->set_temp("last_damage_from",me);
                target->apply_condition("sheng_si", 81*60);  
                target->update_condition("sheng_si");
                ...
                {
                message_vision(HIR"\n$N背心"+HIG"“神道穴”"+HIR"好像被什么人踢了一脚，身子竟朝前跌到，好在$N急定身形，这才稳住脚下的步法。\n"NOR, target);
                target->set_temp("last_damage_from",me);
                target->apply_condition("sheng_si", 81*60);  
                target->update_condition("sheng_si");
                ...
                {
                message_vision(HIW"\n$N后腰"+HIG"“志室穴”"+HIW"觉得麻痒难当，直如千千万万只蚂蚁同时在咬啮一般！\n"NOR, target);
                target->set_temp("last_damage_from",me);
                target->apply_condition("sheng_si", 81*60); 
