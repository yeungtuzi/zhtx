...
int main(object me, string arg)
{
	...
	wiz_only = ({"invisibility", "immortal", "combat", "trace_error"});
	...
	env = me->query("env");
        ...
        for(i=0;i<sizeof(arg);i++)
             if(arg[i]==47) return notify_fail("ＦＡＩＮＴ，你又想给我找麻烦？\n");
	...
	if( term && term!="" ) {
		...
		sscanf(data, "%d", data);
        if((wiz_level(me) == 0&&raw_wiz_level(me)==0) && (member_array(term, wiz_only) != -1))
