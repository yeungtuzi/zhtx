...
//        seteuid(getuid());
        command("say 我的法术等级是"+level+"\n");
	zombie = new("/obj/npc/zombie");
        zombie->set_name( (string)query("victim_name") + "的僵尸", ({ 
"zombie" }) );

        zskill = level;
        if(zskill < 1) zskill = 1;
        zombie->set_skill("unarmed", zskill);
        zombie->set_skill("dodge", zskill);
    	zombie->set("combat_exp",query("combat_exp")+zskill*zskill*zskill/5);
        zombie->set_skill("ghoststrike",zskill);
        zombie->map_skill("unarmed","ghoststrike");
        zombie->set("force_factor",zskill/2);
        zombie->set("max_force",query("max_force")+1000);
        zombie->set("force",query("max_force"));
