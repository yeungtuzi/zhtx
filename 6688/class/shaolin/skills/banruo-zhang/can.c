...
int perform(object me, object target)
{
        ...
        if( me->query_skill_mapped("strike") != "banruo-zhang" )
                return notify_fail("只有般若掌才可以使残生掌。\n");
...
{
        int t;
        t=(int)me->query_skill("banruo-zhang")/5+(int)me->query("age");
        if (!me)
        {
        me->delete_temp("skill_mark/banruo-zhang");
        return 1;
...
{
        int t;
        t=(int)me->query_skill("banruo-zhang")/4+(int)me->query("age");
        if (!me)
        {
        me->delete_temp("skill_mark/banruo-zhang");
        return 1;
...
{
        int t;
        t=(int)me->query_skill("banruo-zhang")/3+(int)me->query("age");
        me->delete_temp("skill_mark/banruo-zhang");
        if (!me) return 1;
        ...
        if (!(objectp(present(target->query("id"), environment(me)))))
        return 1;
        me->add_temp("apply/damage", 3*t);
        me->add_temp("apply/attack", 3*t);
