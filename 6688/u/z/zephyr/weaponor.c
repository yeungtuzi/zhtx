...
YEL"有贼子不知如何得知我收藏兵器之处，趁我不在，将我那五柄最为珍视的神兵盗走。可恨！！\n"NOR,
CYN"说到这里，兵器贩子眼中放出惊人的光芒...\n"NOR,
CYN"你大气都不敢出...\n"NOR,
YEL"“七十年后我出关之时，那些贼子早已经死尽，那五柄神兵也已经各有其主”\n"NOR,
YEL"“唉...”叹毕，兵器贩子不再言语。\n"NOR,
});
...
int accept_object(object ob, object obj)
{
	...
	if ((obj->query("id")=="bo dao")&&(ob->query_temp("weaponor5")))
	{ 
		command("?");
		command("say 这...此刀散发着灵空剑的灵气，但但，怎么会是把刀呢？！\n");
		message_vision("$N向兵器贩子详细的解释了其中变故。\n",environment());
		command("hmm");
		command("say 居然会有此事。\n");
		command("say 灵空剑受此变故，性质也已经有了极大的变化。君是有缘人啊。\n");
		command("say 多谢！\n");
		command("bow");
		ob->set_temp("find_lingkongjian",1);
		ob->set_temp("find_bodao",1);
	...
	if ((obj->query("id")=="chubi blade")&&(ob->query_temp("weaponor5")))
	{
		command("say 果然是名刀出跸！\n");
		command("say 多谢这位英雄！\n");
		command("bow");
		ob->set_temp("find_chubidao",1);
		ob->set_temp("find_intact_chubidao",1);
		if ( ob->query_temp("find_zhantiejian") && ob->query_temp("find_qixiujian") && ob->query_temp("find_yiludao") && ob->query_temp("find_lingkongjian") && ob->query_temp("find_chubidao") )
		{
			call_out("reward", 1, ob);
		}
	...
	if ((obj->query("id")=="chubi dao")&&(ob->query_temp("weaponor5")))
	{
		command("say 果然是名刀出跸！\n");
		command("bow");
		command("say 且慢....\n");
		message_vision("兵器贩子仔细端详了一会出跸刀。良久，他抬起头来，双眼射出摄人的光芒，直盯着$N。\n",environment());
		command("say 原来此刀竟有此经历。它本该随其原主人完成一项重大事务，事务尚未完成，刀怎么就到了你手中？\n");
		command("say 这位英雄，我虽希望此刀能重归我所有，但是，它那尚未完成之事务乃是它于尘世中的缘分，不可强求。\n");
		command("say 如此，老夫暂且不想收回此刀，还请英雄将其规还它的原主人，等它的俗务了了，再请归还于我不迟。\n");
		return 0;
	}
...
int reward(object ob)
{
	...
	if ( ob->query_temp("find_bodao") && !ob->query_temp("find_intact_chubidao") )
	{
		bingqipu=new("/u/z/zephyr/gil_amulet_2");
		ob->set("bingqipu_bodao",1);
	}

	if ( ob->query_temp("find_intact_chubidao") && !ob->query_temp("find_bodao") )
	{
		bingqipu=new("/u/z/zephyr/gil_amulet_3");
		ob->set("bingqipu_intact_chubidao",1);
	}

	if ( ob->query_temp("find_intact_chubidao") && ob->query_temp("find_bodao") )
	{
		bingqipu=new("/u/z/zephyr/gil_amulet_4");
		ob->set("bingqipu_bodao_chubidao",1);
	}
	...
	ob->delete_temp("find_bodao");
	ob->delete_temp("find_chubidao");
	ob->delete_temp("find_intact_chubidao");
