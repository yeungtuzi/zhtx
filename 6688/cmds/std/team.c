...
int main(object me, string arg)
{
	...
	if( arg=="dismiss" ) {
		...
		if( me->is_team_leader() ) {
			i = sizeof(t);
			while(i--)
				if(objectp(t[i])) t[i]->delete_temp("pending/team");
	...
	if( sscanf(arg, "with %s", arg)==1 ) {
		...
		if( !pointerp(me->query_team()) || me->is_team_leader() ) {
			if(arrayp(obj=ob->query_temp("pending/team"))
				&& member_array(me,obj)!=-1) {
